...
利用$A_\mu$的$\a p\a p^\dagger$展开, 我们还可以推导有质量矢量场的LSZ公式. 类似地, 根据引理
\begin{align}
    i\int\d^4x\exp{ipx}(\Box+m^2)A_\mu\epsilon_\lambda^{\mu*}=\sqrt{2\om p}(a_{\lambda\vec p}(+\infty)-a_{\lambda\vec p}(-\infty)).
\end{align}
则我们可以得到
\begin{theorem}[LSZ公式(有质量矢量场)]\label{LSZformula-massive-vector}
    \begin{align}
        \braket{f, +\infty|i, -\infty}&=\int\d^4x_1 i\exp{-ip_1x_1}(\Box_1+m^2)\epsilon^{\mu_1}_{\lambda_1}\cdots\int\d^4x_n i\exp{ip_nx_n}(\Box_n+m^2)\epsilon^{\mu_n*}_{\lambda_n}\notag\\
...
为了得到电磁场的二次量子化结果, 自然的想法就是对电磁场取$m\rightarrow0$的结果. 然而这会面临一个问题: 电磁场的$A^\mu$具有规范不变性, 即$A^\mu\rightarrow A^\mu+\partial^\mu\Lambda$不改变其物理意义, 经过规范变换$\partial_\mu A^\mu$也不一定为0. 这使得我们对电磁场的描述存在冗余自由度, 这的一个直接结果就是电磁场只有两个极化方向而不是重电磁场的三个.
...
\begin{equation}
    \sum_s\epsilon_{si}(\vec p)\epsilon_{sj}(\vec p)=\delta_{ij}-\frac{p_ip_j}{\vec p^2}, 
\end{equation}
于是
\begin{equation}
    \braket{0|A_i(x)A_j(y)|0}=\int\ld p\left(\delta_{ij}-\frac{p_ip_j}{\vec p^2}\right)\exp{-ip(x-y)}.
\end{equation}
所以
\begin{equation}
    \Theta(t_x-t_y)\braket{0|A_i(x)A_j(y)|0}=\int i\dddd p\frac{\exp{-ip(x-y)}}{2|\vec p|(\omega-|\vec p|+i\epsilon)}\left(\delta_{ij}-\frac{p_ip_j}{\vec p^2}\right),
\end{equation}
于是有Feynman传播子(但是并非下小节的Feynman规范的传播子)
\begin{align}
    \braket{0|\mathcal TA_i(x)A_j(y)|0}&=\int\dddd p\frac{i(\delta_{ij}-\frac{p_ip_j}{\vec p^2})}{p^2+i\epsilon}\exp{-ip(x-y)},
\end{align}
并且还有
\begin{align}
...
\begin{align}
    Z[J]=\int\mathcal DA\exp{i\int\d^4x(-\frac14 F^2+J\cdot A)}, 
\end{align}
由于
\begin{align}
    \det(g_{\mu\nu}\partial^2-\partial_\mu\partial_\nu)=0, 
\end{align}
所以这个积分是发散的. 原因在于场$A$的选取具有规范自由度$A_\mu\to A_\mu+\partial_\mu\alpha$, 而在$\alpha$这个自由度上进行积分只是纯粹的累加, 并不会在$\rm{exp}$中贡献Gaussian压低, 进行无穷次累加后就会导致发散. 我们解决这一问题的方法在于将规范冗余的自由度提取出来, 将其变为与后面规范固定的部分相互独立的部分. 而所谓的提取, 就是将$\alpha$部分的路径积分单独提取出来写为一个$\int\mathcal D\alpha$.

我们记$A\to A+\partial\alpha$为$\prescript{\alpha}{}{A}$, 即
\begin{align}
    \prescript{\alpha}{}{A_\mu}\equiv A_\mu+\partial_\mu\alpha.
\end{align}
...
其中
\begin{align}
    \Delta(A)=\left|\frac{\delta f}{\delta\alpha}\right|_{\alpha=0}.
\end{align}
则可以将积分改写为
\begin{align}
    Z[J]=\int\mathcal D\alpha\int\mathcal DA\Delta(A)\delta(f(\prescript{\alpha}{}{A}))\exp{iS},
\end{align}
再利用
\begin{align}
    \mathcal D(A+\partial_\mu\alpha)&=\mathcal DA,\\
    \Delta(A)^{-1}&=\int\mathcal D\alpha'\delta(f(\prescript{\alpha'}{}{A}))=\int\mathcal D(\alpha'+\alpha)\delta(f(\prescript{\alpha'+\alpha}{}{A}))\notag\\
    &=\int\mathcal D\alpha'\delta(f(\prescript{\alpha'+\alpha}{}{A}))=\Delta^{-1}(\prescript{\alpha}{}{A}),
\end{align}
我们有
\begin{align}
    Z[J]&=\int\mathcal D\alpha\int\mathcal D\prescript{\alpha}{}{A}\Delta(\prescript{\alpha}{}{A})\delta(f(\prescript{\alpha}{}{A}))\exp{iS}\\
    &=\int\mathcal D\alpha\int\mathcal DA\Delta(A)\delta(f(A))\exp{iS}.
\end{align}
需要注意, 这里$S$在$A\to\prescript{\alpha}{}{A}$的变换下不变, 是因为我们要求$J^\mu$是守恒流: $\partial_\mu J^\mu=0$.

然后我们考虑一个具体的规范固定方式, 比如选择
\begin{align}
    f(A)=\partial_\mu A^\mu-\omega(x)=0,
\end{align}
则根据
\begin{align}
    f(\prescript{\alpha}{}{A})=\partial_\mu A^\mu+\Box\alpha-\omega(x)=0,
\end{align}
我们有
\begin{align}
    \Delta(A)=\left|\frac{\delta f}{\delta\alpha}\right|_{\alpha=0}=|\Box|.
\end{align}
可以发现$\Delta(A)$与$A$是无关的. 不过考虑Non-Abelian的Yang-Mills理论, 则$\Delta(A)$是包含$A$的, 我们对此的处理会更加复杂一些. 

于是有
\begin{align}
    Z[J]=\int\mathcal D\alpha\int\mathcal DA\Delta(A)\delta(\partial_\mu A^\mu-\omega)\exp{iS}.
\end{align}
由于$\rm{exp}$外面还有一个$\delta$函数, 这让我们非常不爽, 我们希望想办法把$\delta$消掉, 对此的解决方法可以是再对$\omega$做一次积分. 由于我们关心的是$\frac{Z[J]}{Z[0]}$, 所以$Z$上整体的因子多少是无所谓的, 于是我们可以直接给原来的路径积分再乘上一个常数因子: $\int\mathcal D\omega\exp{i\int\d^4x(-\frac{\omega^2}{2\xi})}$, 其中$\xi$为某一常数. 这里引入一个Gaussian分布是为了压制积分使之收敛. 再加上$\int\mathcal D\alpha$已经完全提取出来了规范自由度, 积分剩下的部分与$\alpha$无关, 因此这也是对$Z$的一个整体常数(虽然是无穷大), 我们可以将之丢掉, 于是最终有配分函数
\begin{align}
    Z[J]=\int\mathcal D\omega\int\mathcal DA\Delta(A)\delta(\partial_\mu A^\mu-\omega)\exp{i\int\d^4x(-\frac{\omega^2}{2\xi})}\exp{iS},
\end{align}
然后先对$\omega$积分我们即可将$\delta$消掉, 并丢掉与$A$无关的$\Delta(A)=|\Box|$, 得到
\begin{align}
    Z[J]=\int\mathcal DA\exp{i\int\d^4x(-\frac14 F^2-\frac{(\partial_\mu A^\mu)^2}{2\xi}+J\cdot A)}.
\end{align}
从而得到规范固定后的Lagrangian
\begin{align}
    \mathcal L_{GF}=-\frac14 F^2-\frac{(\partial_\mu A^\mu)^2}{2\xi}+J\cdot A=\frac12 A^\mu(g_{\mu\nu}\partial^2-(1-\frac1\xi)\partial_\mu\partial_\nu)A^\nu+J_\mu A^\mu.
\end{align}
...
其逆为Green函数$G^{\mu\nu}$:
\begin{align}
    G^{\mu\nu}(x-y)=\int\dddd p\frac{-g^{\mu\nu}+(1-\xi)p^\mu p^\nu/p^2}{p^2+i\epsilon}\exp{-ip(x-y)},
\end{align}
于是
\begin{align}
    S&=\frac12\braket{A^\mu, O_{\mu\nu}A^\nu}+\braket{J_\mu, A^\mu}\\
    &=\frac12\braket{A^\mu+G^{\mu\alpha}J_\alpha, O_{\mu\nu}(A^\nu+G^{\nu\alpha}J_\alpha)}-\frac12\braket{J_\mu, G^{\mu\nu}J_\nu}.
\end{align}
设光子传播子为
\begin{align}
    D_{\mu\nu}(x-y)\equiv iG_{\mu\nu}(x-y)=\int\dddd p\frac{-i}{p^2+i\epsilon}(g_{\mu\nu}-(1-\xi)p_\mu p_\nu/p^2)\exp{-ip(x-y)},
\end{align}
我们有
\begin{align}
    Z[J]=Z[0]\exp{-\frac12\int\d^4x\d^4yJ^\mu(x)D_{\mu\nu}(x-y)J^{\nu}(y)},
\end{align}
从而有
\begin{align}
    \braket{A_\mu(x)A_\nu(y)}=\frac{-1}{Z[0]}\frac{\delta}{\delta J(x)}\frac{\delta}{\delta J(y)}Z[J]\Big|_{J=0}=D_{\mu\nu}(x-y).
\end{align}

我们可以利用定理\ref{ward-identity-qed}证明, 对于on-shell的连通图, Ward恒等式保证了$\xi$的不同选取不会影响可观测量.

需要备注, 对于经典电动力学, 我们也是可以使用$\xi$规范固定下来的
\begin{align}
    \mathcal L_{GF}=-\frac14 F^2-\frac{(\partial_\mu A^\mu)^2}{2\xi}+J\cdot A\notag,
\end{align}
进行计算.

\subsection{规范矢量场的经典相互作用势}
这里我们利用类似\ref{pi-yukawa}节的方法, 计算验证我们导出的光子传播子确实符合我们熟知的经典Coloumb势
\begin{align}
    U(r)=\frac{e_1e_2}{4\pi r}.
\end{align}

考虑两带电粒子$q_1, q_2$, 电流为
\begin{align}
    J^\mu(x)=q_1\int\d\tau\de{y_1^\mu}{\tau}\delta^4(x-y_1)+q_2\int\d\tau\de{y_2^\mu}{\tau}\delta^4(x-y_2).
\end{align}
于是我们有
\begin{align}
    S_{\rm{eff}}&=\frac i2\int\d^4x_1\d^4x_2J^\mu(x_1)D_{\mu\nu}(x_1-x_2)J^\nu(x_2)\\
    &=iq_1q_2\int\d\tau_1\int\d\tau_2\de{y_1^\mu}{\tau}\de{y_2^\nu}{\tau}D_{\mu\nu}(y_1-y_2),
\end{align}
我们继续假设带电粒子静止, 并且$\vec r=\vec y_1-\vec y_2$, 于是有
\begin{align}
    S_{\rm{eff}}&=iq_1q_2\int\d t_1\int\d t_2 D_{00}(y_1-y_2)\\
    &=q_1q_2\int\d t_1\d t_2\int\dddd p\frac{g_{00}-(1-\xi)p_0p_0/p^2}{p^2+i\epsilon}\exp{-i(\omega(t_1-t_2)-\vec p\cdot(\vec y_1-\vec y_2))},
\end{align}
对$\d t_1$积分得到一个$\delta(\omega)$, 再消掉一个$\d p$, 代入$\omega=0$, 我们有,
\begin{align}
    S_{\rm{eff}}&=-q_1q_2\int\d t\int\ddd p\frac{\exp{i\vec p\cdot\vec r}}{\vec p^2}\\
    &=-\int\d t\frac{q_1q_2}{4\pi r}\\
    &=-\int\d t U(r).
\end{align}

从而我们有结论
\begin{align}
    U(r)=\frac{q_1q_2}{4\pi r}\notag,
\end{align}
这符合我们的预期.
...
\begin{align}
    \begin{cases}
        &(\partial^2+m^2)\psi=-2ieA_\mu\partial^\mu\psi-ie(\partial_\mu A^\mu)\psi+e^2\psi A_\mu A^\mu\\
        &\partial_\mu F^{\mu\nu}=ie(\psi\partial^\nu\psi^*-\psi^*\partial^\nu\psi-2ie\psi\psi^*A^\nu)
    \end{cases},
\end{align}
...
\subsection{标量QED的Feynman规则}\label{SQED-feynman}
我们不难写出Dyson级数
\begin{align}
...
\end{equation}
而我们的$\partial_\mu$就是将传播子求导, 因此也就是提取出来$\pm ip_\mu$, 但是正负号需要我们仔细考虑, 也就是说, 我们在写Feynman规则的时候, 应当标定顶点上预期的动量流向, 然后Feynman规则中的正负号可以利用LSZ公式来确定. 我们不妨选令动量流向与粒子流的方向一致, 于是我们可以发现$\partial_\mu\psi=-ip_\mu\psi$, $\partial_\mu\psi^\dagger=ip_\mu\psi^\dagger$. 这样就有Feynman规则,
\begin{enumerate}
    \item 复标量传播子
    $$
...
\begin{example}[pion散射]
    ...
    从而计算得到质心系中的散射截面
    \begin{equation}
        \left(\frac{\d\sigma}{\d\Omega}\right)_{CM}=\frac{e^4}{64\pi^2E_{CM}^2}\left(\frac{u-t}s+\frac{u-s}t\right)^2
    \end{equation}
\end{example}
