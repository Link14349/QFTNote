...
\begin{definition}[连续对称性]
    ...
    \end{equation}
    其中, $\Delta$为某一算符. 若系统具有该变换的对称性, 则运动方程应该不变, 即:
    \begin{equation}
...
\songti
这里比较令人难以理解的其实是$J^\mu$, 这看着像是一个任意的, 我们不知道的量, 那我们又该如何确定Noether流呢? 其实是$J^\mu$是具有一个确定形式的. 对于内禀自由度的变换来说, 我们可以明确验证$\delta L=0$, 因此$J^\mu=0$; 而对于时空变换自由度来说, $J^\mu$是可以通过变换的Killing矢量场$\xi^\mu$完全确定的, 并且$J^\mu=-\xi^\mu\mathcal L$, 接下来我们将给出定理来具体介绍.

\begin{theorem}
    考虑流形$M$上的一个微分同胚$\psi: M\to M$, 对于其诱导的推出$\psi_*$、拉回映射$\psi^*$, 有
    \begin{align}
        \mathcal L(\psi^*\phi, \psi^*(\partial_a\phi))=\psi^*\mathcal L(\phi, \partial_a\phi).
    \end{align}
\end{theorem}
\begin{proof}
    \begin{align}
        \mathcal L(\psi^*\phi, \psi^*(\partial_a\phi))\big|_p&=\mathcal L(\psi^*\phi, \partial_a\psi^*(\phi)|_p)\big|_p\\
        &=\mathcal L(\phi(\psi(p)), \partial_a\phi(\psi(p)))\\
        &=\mathcal L(\phi, \partial_a\phi)\big|_{\psi(p)}\\
        &=\psi^*\mathcal L\big|_p.
    \end{align}
\end{proof}
利用这个定理, 对于无穷小微分同胚变换, 我们有
\begin{align}
    \delta\mathcal L=-\xi^\mu\partial_\mu\mathcal L,
\end{align}
对于Killing矢量场$\partial_\mu\xi^\mu=0$, 于是
\begin{align}
    \delta\mathcal L=\partial_\mu(-\xi^\mu\partial_\mu\mathcal L),
\end{align}
从而可以得到
\begin{align}
    J^\mu=-\xi^\mu\mathcal L.
\end{align}

% 读者可能会质疑, 我们这里的推导证明要求了$\xi^\mu$是保度规的微分同胚, 如果它不保微分同胚, 允许度规张量差一个共形因子呢? 似乎推导就不成立了? 这其实依然成立, 原因在于, 在引入共形对称性的时候, 因为度规张量的行列式$g$在变换后就不一定依然为常数$1$了, 而最一般的作用量的表达式为
% \begin{align}
%     S=\int\mathcal L=\int\d^4x\sqrt g\mathcal L,
% \end{align}
% 因此我们在考虑对称性时, 应当自然耦合引力, 考虑$\sqrt{\sqrt g\mathcal L}$整体的变换. 利用共形变换中
% \begin{align}
%     \delta\sqrt g=-\partial_a\xi^a\sqrt g, 
% \end{align}
% 我们得到
% \begin{align}
%     \delta(\sqrt g\mathcal L)=\sqrt g\left(-\partial_a\xi^a\mathcal L+\delta\mathcal L\right)=\sqrt g\left(-\partial_a(\xi^a\mathcal L)+\delta\mathcal L+\xi^a\partial_a\mathcal L\right).
% \end{align}
% 其中第一项为一个全微分(不需要将$\sqrt g$放入全微分, 因为Gauss定理为$\int\nabla_a v^a=\int v^a n_a$, 写到具体坐标系中本来就在外面乘了一个测度$\sqrt g$), 如果系统具有对称性, $\delta(\sqrt g\mathcal L)$不能有全微分以外的项, 因此剩下的两项应当为$0$, 于是仍然有
% \begin{align}
%     \delta\mathcal L=-\xi^\mu\partial_\mu\mathcal L.
% \end{align}
...
\begin{example}[复标量场的$U(1)$对称性]\label{csU1}
    ...
    于是, 由Noether定理可得守恒流
    \begin{equation}
        j^\mu = i(\psi\partial^\mu\psi^* - \psi^*\partial^\mu\psi)\label{complex-scalar-u1-current}
    \end{equation}
    我们可以代入验证其守恒:
    \begin{equation}
...
\begin{example}[矢量场的时空平移对称性]
    类似例\ref{ex:ct_scalar_translation}, 时空中有Killing矢量场$\xi^\mu$.
    我们首先设两个辅助场:
    \begin{align}
...
\end{example}
\begin{example}[Dirac场的时空平移对称性]
    在推导Dirac场的时空平移对称性时, 为了具有一般性, 最大的难点在于给出旋量在一般的微分同胚群作用下的变换, 我们在此不做推导(我也不会推导...). 根据的Kosmann 1971年的成果\cite{LieSpinor}, 我们有对Dirac旋量场$\psi$的Lie导数表达式
    \begin{equation}
        \mathscr L_\xi\psi=\xi^a\nabla_a\psi-\frac14\nabla_{[a}\xi_{b]}\gamma^a\gamma^b\psi
    \end{equation}
    其中$\xi^a$为Killing矢量场.

    如果你不放心, 我们可以验证后面拖拽项就是对旋量的Lorentz变换的正确性: 对于Minkovski时空中, 一般的Killing矢量场为
    \begin{equation}
        \xi^\mu=b^\mu+\omega^\mu_{~~\nu} x^\nu
    \end{equation}
    所以
    \begin{align}
        \mathscr L_\xi\psi&=\xi^\mu\nabla_\mu\psi+\frac14\omega_{\mu\nu}\gamma^\mu\gamma^\nu\psi\\
        &=\xi^\mu\nabla_\mu\psi-\frac i2\omega_{\mu\nu} \frac i4[\gamma^\mu, \gamma^\nu]\psi\\
        &=\xi^\mu\nabla_\mu\psi-\frac i2\omega_{\mu\nu}S^{\mu\nu}\psi
    \end{align}
    
    于是我们发现对于Killing矢量场, 拖拽项确实是对旋量的Lorentz变换.

    然后我们尝试推导能动张量. (似乎有点不太对, 等我再思考酝酿下)
\end{example}

上面的讨论有一个问题是, 我们如何确定$J^\mu$的形式, 有些时候它可能并不是一个很好确定的东西. 所以下面我们给出Noether第二定理, 或者说Noether's Trick, 它可以一般性的确定任意对称性的守恒流.

考虑一个任意的变换
\begin{align}
    & x^\mu\to x'^\mu=x^\mu+\epsilon\xi^\mu,\notag\\
    & \phi(x)\to \phi'(x')=\phi(x)+\mathcal F[\phi],\label{arbitary-field-trans},
\end{align}
我们计算变换后的作用量
\begin{align}
...
如果这是一种对称性变换, 则$S$在这个变换下不变$S[\phi]=S[\phi']$. 这里我们使用所谓的Noether's Trick, 也就是将全局的参数$\epsilon$提升为局域的参数$\epsilon(x)$, 则$S[\phi']$一定会改变, 并且正比于$\epsilon$的项一定为$0$, 于是最低阶的也一定是正比$\partial_\mu\epsilon$的, 我们设
\begin{align}
    S[\phi']=S[\phi]-\int\d^4x j^\mu\partial_\mu\epsilon(x).
\end{align}
我们要求在$x\to\infty$时, $\epsilon(x)\to0$, 则分部积分有
\begin{align}
    S[\phi']=S[\phi]+\int\d^4x\epsilon(x)\partial_\mu j^\mu.\label{noether-trick},
\end{align}

然后我们考虑满足EOM的场配置, 因为对于满足EOM的场, 对其做任意的无穷小变换$\delta\phi$, 都有$\delta S=0$, 而$\delta\phi=\mathcal F[\phi]-\epsilon\xi^\mu\partial_\mu\phi$本身就是一个无穷小的场改变. 于是对于\textbf{满足EOM}的场, 同样有
\begin{align}
    S[\phi']=S[\phi],
\end{align}
于是
\begin{align}
    \int\d^4x\epsilon(x)\partial_\mu j^\mu[\phi^{EOM}]=0, 
\end{align}
又因为$\epsilon(x)$是完全任意的, 所以
\begin{align}
    \partial_\mu j^\mu[\phi^{EOM}]=0.
\end{align}
于是这样我们就得到了守恒流.

对于如式\eqref{arbitary-field-trans}那样的场变换, 我们可以直接给出具体计算
\begin{align}
    \partial'_\mu\phi'(x')&=(\partial_\mu-\partial_\mu(\epsilon\xi^\nu)\partial_\nu)\phi'(x')\\
    &=(\partial_\mu-\partial_\mu(\epsilon\xi^\nu)\partial_\nu)(\phi(x)+\mathcal F[\phi])\\
    &=\partial_\mu\phi+\partial_\mu\left(\epsilon\mathcal F\phi\right)-\partial_\mu(\epsilon\xi^\nu)\partial_\nu\phi,
\end{align}
从而我们有
\begin{align}
    & \delta\phi'(x)=\mathcal F[\phi]-\epsilon\xi^\mu\partial_\mu\phi,\\
    & \delta\partial_\mu\phi(x)=\partial_\mu\left(\epsilon\mathcal F\phi\right)-\partial_\mu(\epsilon\xi^\nu)\partial_\nu\phi.
\end{align}
于是
\begin{align}
    \delta S&=\int\d^4x'\mathcal L(\phi'(x'), \partial'\phi'(x'))-\int\d^4x\mathcal L(\phi(x), \partial\phi(x))\\
    &=\int\d^4x\left[\left(\left|\pa{x^\mu}{x'^\nu}\right|-1\right)\mathcal L+\pa{\mathcal L}{\phi}\delta\phi+\pa{\mathcal L}{(\partial_\mu\phi)}\delta\partial_\mu\phi\right].
\end{align}
注意其中第一项为
\begin{align}
    \left|\pa{x^\mu}{x'^\nu}\right|-1=\partial_\mu(\epsilon\xi^\mu), 
\end{align}
代表测度本身的改变. 只提取出正比$\partial_\mu\epsilon$的项, 我们得到
\begin{align}
    \delta S=-\int\d^4x\partial_\mu\epsilon\left[-\xi^\mu\mathcal L+\Pi^\mu\left(\xi^\nu\partial_\nu\phi-\mathcal F[\phi]\right)\right].
\end{align}
于是我们可以得到守恒流
\begin{align}
    j^\mu=-\xi^\mu\mathcal L+\Pi^\mu\left(\xi^\nu\partial_\nu\phi-\mathcal F[\phi]\right),
\end{align}
其中
\begin{align}
    \Pi^\mu=\pa{\mathcal L}{(\partial_\mu\phi)}.
\end{align}
利用能动张量定义
\begin{align}
    T_{\mu\nu}=\Pi_\mu\partial_\nu\phi-g_{\mu\nu}\mathcal L, 
...
我们有
\begin{align}
    j_\mu=\xi^\nu T_{\mu\nu}-\Pi_\mu\mathcal F[\phi].\label{general-sym-current}
\end{align}
...
\subsection{自由实标量场的量子化\label{2nd_quant_free_real_scalar}}
% \begin{enumerate}
...
\begin{equation}
    \omega^2=\vec p^2+m^2,
\end{equation}
满足这个色散关系的, 或者满足EoM的粒子称为称为on-shell的粒子.

我们可以计算出正则动量
\begin{align}
    \pi=\pa{\mathcal L}{\dot\phi}=\dot\phi,
\end{align}
于是我们可以得到哈密顿量
\begin{equation}
...
我们还可以定义动量算子:
\begin{equation}
    P^i=\int\partial^0\phi\,\partial^i\phi\,\mathrm d^3x=-\int\phi_t\phi_i\mathrm d^3x,
\end{equation}
即:
\begin{align}
...
\end{align}
\begin{align}
    \pi=\pi^0=\partial^0\psi^*=\partial_0\psi^*,
\end{align}
...
设
\begin{align}
    &\a p=\dpi3\sqrt{\frac{\om p}2}(\Psi_\vec p+\frac i{\dpi3\om p}\Pi^\dagger_{-\vec p})=\frac1{\sqrt{2\om p}}\int\d^3x\exp{ipx}(\om p\psi+i\pi^\dagger)\\
    &\b p=\dpi3\sqrt{\frac{\om p}2}(\Psi_{-\vec p}^\dagger+\frac i{\dpi3\om p}\Pi_{\vec p})=\frac1{\sqrt{2\om p}}\int\d^3x\exp{-ipx}(\om p\psi^\dagger+i\pi),
\end{align}

我们有:
\begin{align}
    [\a p, \a q^\dagger]&=\dpi3\delta^3(\vec p-\vec q), \quad [\a p, \a q]=[\a p^\dagger, \a q^\dagger]=0\\
    [\b p, \b q^\dagger]&=\dpi3\delta^3(\vec p-\vec q), \quad [\b p, \b q]=[\b p^\dagger, \b q^\dagger]=0,
\end{align}
...
\begin{equation}
    Q\equiv\int d^3x J_N^0=\int d^3xi\left[(\partial^0\psi^\dagger)\psi-(\partial^0\psi)\psi^\dagger\right],
\end{equation}
...
并且有对易关系
\begin{align}\label{complex-scalar-Q-commutation-relations}
    &[Q_N, \psi_x]=\psi_x\\
    &[Q_N, \psi_x^\dagger]=-\psi_x^\dagger,
\end{align}
...
$$
    \braket{\phi_1\cdots\phi_n}.
$$
这个定义可能比较唐突, 我们难以理解为什么我们需要在场论中计算关联函数, 为什么关联函数非常重要. 因此在具体计算之前我们想给出一些具体的关联函数的意义的例子, 说明它的重要性和物理意义.

我们知道, 某一个态可以通过场算符激发出来
\begin{align}
    \ket{X}=\phi(x)\cdots\ket\Omega,
\end{align}
经过演化后一个态到另一个态的概率振幅为
\begin{align}
    \braket{Y|X}=\braket{\Omega|\phi(y)\cdots\phi(x)|\Omega},
\end{align}
某个可观测量测量的平均值为
\begin{align}
    \braket{O}=\braket{X|O|X}=\braket{\Omega|\cdots\phi(x)O\phi(x)\cdots|\Omega},
\end{align}
可以看到, 这就是一个关联函数的形式. 在此基础之上, 因为, 微分散射截面、衰变率甚至能流分布等物理量本质上就是通过这两个表达式来表达, 因此也可以通过关联函数算出来.

这是我们常见的粒子物理中的理解, 但是其可应用的范围远远不止于此. 在腾飞的讲义中给出了两个更加有意思的例子\cite{gft:lecture1}: 其一是黑洞散射发射出的引力波波形的计算, 可以表达为
$$
    h_{\mu\nu}(\tau, \theta, \phi)=\braket{\rm{in}|S^\dagger h_{\mu\nu} S|\rm{in}}_{\text{classical limit}}
$$
也就是说, 就连经典场的散射我们原则上也是可以利用QFT关联函数计算的框架最终取经典极限$\hbar\to0$得到! 

再一个则是宇宙微波背景辐射的关联函数: 微波背景辐射近似是各向同性的, 这一点可以通过暴涨模型很好地解释(宇宙尺度很大, 但是可以让其中的光子气处于热平衡态, 只能是在宇宙很小的时候宇宙两点间相互作用很强的时代就已经达成了平衡). 但是实际上更精细的实验指出, 它实际上是存在微小涨落的, 而这个涨落的平均正是关联函数, 并且可以通过弯曲时空QFT计算出来!
$$
    \braket{\Omega|\phi(x_1)\phi(x_2)\phi(x_3)\phi(x_4)|\Omega}
$$

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{./image/cmb.png}
    \caption{微波背景辐射图}
\end{figure}

\subsubsection{实标量场}\label{2pt-real-scalar}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \end{tikzpicture}
        \caption{$x^0>y^0$的围道}
        \label{fig:q1f1a-greater}
    \end{subfigure}
    \caption{$D_R(x-y)$围道示意图}
    \label{fig:q2f1}
\end{figure}

为了让大圆弧不会对积分结果产生贡献, 对于$x^0<y^0$我们取围道如图\ref{fig:q1f1a-less}; 对于$x^0>y^0$我们取围道如图\ref{fig:q1f1a-greater}.
...
\begin{theorem}[提前关联函数$D_A(x-y)$]
    \begin{equation}
        D_A(x-y)\equiv\lim_{\epsilon\rightarrow0^+}\int\dddd p \frac i{(p-i\epsilon)^2-m^2}\exp{-ip(x-y)}\label{q2eq4}
    \end{equation}
\end{theorem}
我们可以发现:
\begin{equation}
...
\begin{theorem}[共轭Feynman传播子$\widetilde{D}_F(x-y)$]
    ...
    % 或者等价于取一个无穷小正数$\epsilon$:
    \begin{equation}
        \widetilde D_F(x-y)\equiv\lim_{\epsilon\rightarrow0^+}\int\dddd p \frac i{p^2-m^2-i\epsilon}\exp{-ip(x-y)}\label{q2eq6}
    \end{equation}
\end{theorem}
...
在量子场论中我们只使用Feynman传播子, 这个原因在后面我们有关Dyson级数以及路径积分的讨论中可以清楚地看到. 为了能让我们对于Feynman传播子的讨论对于接下来的工作更有推广借鉴意义, 在此我们给出另一种从关联函数定义
\begin{equation}
    D_F(x, y)=\braket{0|\mathcal T\phi_x\phi_y|0}=\Theta(t_x-t_y)\braket{0|\phi_x\phi_y|0}+\Theta(t_y-t_x)\braket{0|\phi_y\phi_x|0},
\end{equation}
出发来得到Feynman传播子的方式, 这个方法在接下来的推导中更为有用. 

根据式\eqref{Theta-definition}我们有
\begin{align}
    D_F(x, y)=&\int\frac{i\d\omega}{2\pi}\frac{\exp{-i\omega(t_x-t_y)}}{\omega+i\epsilon}\int\ld p[\exp{-ip(x-y)}]\notag\\
    &+\int\frac{i\d\omega}{2\pi}\frac{\exp{i\omega(t_x-t_y)}}{\omega+i\epsilon}\int\ld p[\exp{ip(x-y)}]\label{scalar-feynman-propagator-naively},
\end{align}
(注意, 这里$p(x-y)=\omega(t_x-t_y)-\vec p\cdot(\vec x-\vec y)$). 我们设
\begin{equation}
    p'^\mu=(\omega, \vec p)^T,
\end{equation}
则有
\begin{equation}
    D_F(x, y)=\int i\dddd p'\frac{\exp{-ip'(x-y)-i\om p(t_x-t_y)}}{(p'^0+i\epsilon)2\om p}+\int i\dddd p'\frac{\exp{ip'(x-y)+i\om p(t_x-t_y)}}{(p'^0+i\epsilon)2\om p},
\end{equation}
(注意, 这里$p(x-y)=p^0(t_x-t_y)-\vec p\cdot(\vec x-\vec y)$, 其中$p^0=\omega$, 因此这里$p(x-y)$与式\eqref{scalar-feynman-propagator-naively}的含义是不同的). 再做换元将$\om p$吸收到$p$中
\begin{equation}
    p^0=p'^0+\om p=p'^0+\sqrt{\vec p'^2+m^2},
\end{equation}
这个换元的Jacobi行列式
\begin{equation}
...
得到
\begin{equation}
    D_F(x, y)=\int i\dddd p\left(\frac{\exp{-ip(x-y)}}{(p^0-\om p+i\epsilon)2\om p}+\frac{\exp{ip(x-y)}}{(p^0-\om p+i\epsilon)2\om p}\right).
\end{equation}
再对第二项做换元, 将$\exp{-ip(x-y)}$提到括号外
\begin{equation}
    p\to -p,
\end{equation}
我们有
\begin{align}
    D_F(x, y)&=\int i\dddd p\frac{\exp{-ip(x-y)}}{2\om p}\left(\frac{1}{p^0-\om p+i\epsilon}+\frac{1}{-p^0-\om p+i\epsilon}\right)\\
    &=\int i\dddd p\exp{-ip(x-y)}\frac1{(p'^0-\om p+i\epsilon)(p'^0+\om p-i\epsilon)}\\
    &=\int i\dddd p\exp{-ip(x-y)}\frac1{(p'^0)^2-\om p^2+2i\om p\epsilon+\epsilon^2},
\end{align}
忽略掉$\epsilon$的二阶项, 并且将$\om p$吸收进$\epsilon$, 利用$\om p^2=\vec p^2+m^2$, 我们有
\begin{align}
    D_F(x, y)&=\int\dddd p\frac i{p^2-m^2+i\epsilon}\exp{-ip(x-y)}.
\end{align}
这就是实标量场的Feynman传播子.
...
我们还是首先计算
\begin{align}
    \braket{0|\psi_x\psi_y^\dagger|0}&=\bra0\int\ldsq p\left(\a p^\dagger\exp{ipx}+\b p\exp{-ipx}\right)\notag\\
    &\;\int\ldsq q\left(\a q\exp{-iqy}+\b p^\dagger\exp{iqy}\right)\ket0\\
    &=\int\ld p[\exp{-ip(x-y)}]\\
    \braket{0|\psi_y^\dagger\psi_x|0}&=\bra0\int\ldsq q\left(\a q\exp{-iqy}+\b p^\dagger\exp{iqy}\right)\notag\\
    &\;\int\ldsq p\left(\a p^\dagger\exp{ipx}+\b p\exp{-ipx}\right)\ket0\\
    &=\int\ld p[\exp{ip(x-y)}],
\end{align}

类似地, 我们可以将$\Theta$展开为Fourier积分, 从而给出复标量场的Feynman传播子
\begin{align}
...
\begin{align}
    \braket{\psi_x\psi_y}=\braket{\psi_x^\dagger\psi_y^\dagger}=0,
\end{align}

这是因为根据$Q\ket0=0$, 即真空态电中性, 我们有
\begin{align}
    \braket{0|\mathcal T\psi_x\psi_y|0}&=\braket{0|\mathcal T\psi_x\psi_y|0}+\braket{0|Q_N\mathcal T\psi_x\psi_y|0}\\
    &=\braket{0|\mathcal T\psi_x\psi_y|0}+\braket{0|\mathcal T\psi_xQ_N\psi_y|0}+\braket{0|\mathcal T\psi_x\psi_y|0}\\
    &=2\braket{0|\mathcal T\psi_x\psi_y|0}+\braket{0|\mathcal T\psi_x\psi_yQ_N|0}+\braket{0|\mathcal T\psi_x\psi_y|0}\\
    &=3\braket{0|\mathcal T\psi_x\psi_y|0},
\end{align}
(需要注意, 因为$Q_N$不含时, 所以可以自由进出$\mathcal T$, 然后利用式\eqref{complex-scalar-Q-commutation-relations}中给出的$Q_N$与$\psi_x$的对易关系). 因此
\begin{align}
    \braket{0|\mathcal T\psi_x\psi_y|0}=0,
\end{align}
同理可得
\begin{align}
    \braket{0|\mathcal T\psi_x^\dagger\psi_y^\dagger|0}=0
\end{align}

\subsubsection{关联函数与Green函数}
...
对于旋量, Wick定理同样满足, 但是在这时因为
\begin{equation}
    \braket{0|\mathcal T\psi_x\bar\psi_y|0}=-\braket{0|\mathcal T\bar\psi_y\psi_x|0}\neq\braket{0|\mathcal T\bar\psi_y\psi_x|0},
\end{equation}
Wick收缩的内顺序(即两个场$\psi, \bar\psi$缩并时谁在前谁在后)是不可忽略的, 不难证明, 正确的缩并方法应该是在缩并内保持$\mathcal T$内的先后顺序.

\subsection{对称性与守恒荷}
考虑某一变换$X$(要求不只是保持$S$不变, 还进一步要求$\mathcal L$不变)
\begin{align}
    \psi_i\to\epsilon\delta_X\psi_i,
\end{align}
我们可以定义出守恒流
\begin{align}
    j^\mu_X=\sum_i\frac{\partial\mathcal L}{\partial(\partial_\mu\psi_i)}\delta_X\psi_i,
\end{align}
并且有守恒荷
\begin{align}
    Q_X&=\int\d^3x j^0_X(x)\\
    &=\int\d^3x\sum_i\frac{\partial\mathcal L}{\partial(\partial_t\psi_i)}\delta_X\psi_i\notag\\
    &=\int\d^3x\sum_i \pi_i(x)\delta_X\psi_i(x).
\end{align}

如果$\delta_X$是commute的, 则我们有如下推导:
对于动量满足对易关系的场,
\begin{align}
    [\psi_i(x), \pi_j(y)]=i\delta_{ij}\delta^3(x-y),
\end{align}
我们有
\begin{align}
    i[Q_X, \psi_i]&=\int\d^3y\sum_j i[\pi_j(y)\delta_X\psi_j(y), \psi_i(x)]\\
    &=\int\d^3y\sum_j i([\pi_j(y), \psi_i(x)]\delta_X\psi_j(y)+\pi_j(y)[\delta_X\psi_j(y), \psi_i(x)]),
\end{align}
若对称性不包含时间上的变换, 则$\delta_X\psi$不含有场对时间的导数, 也就不含正则动量, 因此在等时条件下有
\begin{align}
    [\delta_X\psi_j(y), \psi_i(x)]=0, 
\end{align}
从而
\begin{align}
    i[Q_X, \psi_i]&=\int\d^3y\sum_j i([\pi_j(y), \psi_i(x)]\delta_X\psi_j(y))\\
    &=\int\d^3y\sum_j i(-i\delta_{ij}\delta^3(x-y)\delta_X\psi_j(y))\\
    &=\delta_X\psi_i(x).
\end{align}
对于动量满足反对易关系的场, 我们可以类似推导得到
\begin{align}
    i[Q_X, \psi_i]&=\int\d^3y\sum_j i(\{\pi_j(y), \psi_i(x)\}\delta_X\psi_j(y)-\pi_j(y)\{\delta_X\psi_j(y), \psi_i(x)\})\\
    &=\int\d^3y\sum_j i(-i\delta_{ij}\delta^3(x-y)\delta_X\psi_j(y))\\
    &=\delta_X\psi_i(x).
\end{align}

而对于anti-commute的$\delta_X$, 比如说BRST的, 则有细微的区别, 对于动量位形是对易子关系的场结论仍然不变
\begin{align}
    i[Q_X, \psi_i]=\delta_X\psi_i(x),
\end{align}
但是对于反对易的算子则有
\begin{align}
    i\{Q_X, \psi_i\}=\delta_X\psi_i(x).
\end{align}
值得注意, 这里$Q_X$是anti-commute的.

于是我们有定理
\begin{theorem}\label{conserved-charge-as-generator}
    考虑一保持$\mathcal L$不变的对称性$X$, 场的无穷小变换为$\psi\to\psi+\epsilon\delta_X\psi$, 对于动量与位形为对易关系的算符, 若它满足在等时条件下, 
    \begin{align}
        [\delta_X\psi_j(y), \psi_i(x)]=0,
    \end{align}
    则
    \begin{align}
        i[Q_X, \psi_i]=\delta_X\psi_i(x);
    \end{align}
    而对于动量位形是反对易的算符, 若$\delta_X$是commute的, 仍然有
    \begin{align}
        i[Q_X, \psi_i]=\delta_X\psi_i(x),
    \end{align}
    但是对于anti-commute的$\delta_X$, 则
    \begin{align}
        i\{Q_X, \psi_i\}=\delta_X\psi_i(x).
    \end{align}
\end{theorem}
...
\begin{align}
    \exp{iQ_X\epsilon}\psi\exp{-iQ_X\epsilon}=X\psi.
\end{align}
这验证了守恒荷确实是这个变换的对应生成元(需要注意, 对于anti-commute的$Q_X$, $\epsilon$也是anti-commute的, 比如BRST对称性就是如此).
