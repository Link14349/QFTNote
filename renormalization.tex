...
从而有修正一阶修正的四点关联函数
\begin{align}
    iM&=-i\lambda+\frac{i\lambda^2}{32\pi^2}\sum_{c=s,t,u}\int_0^1\dx\left[\ln\frac{\Delta(c)+\Lambda^2}{\Delta_0(c)+\Lambda^2}\frac{\Delta_0(c)}{\Delta(c)}\right.\notag\\
...
根据前面的分析, 我们知道重点在于提取处$(p+k)^\mu$前面的系数, 因此我们只算$A^\mu$中$p^\mu$与$k^\mu$的系数. 不难得到
\begin{align}
...
\end{align}
裸的$\phi_{bare}$是直接出现在在Lagrangian中的, 与$\mu$的选取是无关的，因此
\begin{align}
...
\begin{align}
    G^{(2)}(p)&=\frac{h(p)}{p^2}, 
\end{align}
其中$h$是一个无量纲的因子. 又因为粒子是无质量的, $h$还无量纲, 那若$h$与$p$有关, 就必须先通过唯一的动量量纲的能标$\mu$将$p$无量钢化, 也就是要求
\begin{align}
