...
我们可以用这些fancy的图来表示这些二阶贡献, 这就是Feynman图:
\begin{figure}[htbp!]
    \begin{tikzpicture}
        \draw (-0.5,0) -- (1,0) ;
        \draw (2,0) -- (3.5,0);
...
注意到, 这里的缩并都完全是自由场的缩并(相互作用绘景下的$\phi_I$就是自由场在Heisenberg绘景下的$\phi_0$). 然后利用Wick定理可以把这些缩并写为自由场传播子$D_F$的乘积与求和. 并且这个求和同样可以用Feynman图写出, 即真空图:
\begin{align}
