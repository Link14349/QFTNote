...
\end{align}
当然, 核函数也可以有其他取法; 更详尽的例子可以见Schwartz\cite{schwartz-kernel}. 比如指数衰减:
\begin{align}
    f(x)=\exp{-x}, 
\end{align}
这称为热核函数.
...
\begin{align}
    ...
    &\;\;-\frac1{30}\frac{F^{(3)}(N)-F^{(3)}(0)}{4!}+\cdots\notag\\
    &\;\;+B_j\frac{F^{(j-1)}(N)-F^{(j-1)}(0)}{j!}+\cdots
\end{align}
...
\begin{align}
    \frac{E_{\rm{tot}}}A&=\frac{\Lambda^3}{2\pi}\left(\sum_n\left[1+\frac{n\pi}{\Lambda a}+\frac12\left(\frac{n\pi}{\Lambda a}\right)^2\right]\exp{-\frac{n\pi}{\Lambda a}}\right.\notag\\
    &\quad\left.-\int_0^{+\infty}\d n\left[1+\frac{n\pi}{\Lambda a}+\frac12\left(\frac{n\pi}{\Lambda a}\right)^2\right]\exp{-\frac{n\pi}{\Lambda a}}\right).
\end{align}
...
\begin{align}
    \int^{+\infty} p^{D-1}\d p,
\end{align}
的积分, 其发散程度为$[\Lambda]^{D}$, $D=0$时为$\ln\Lambda$. 这里$D$即我们想要估算的量纲参数: 表面发散程度(superficial degree of divergence). 但是需要注意, 它并不一定能完全表征图的发散程度: 子图的发散可能会让$D$算出来不发散的图也发散; 规范对称性可能提取出整体的动量使得$D$算出来发散的图实际上不发散. 但是作为一个粗糙的估计也暂时足够了.
...
这里把四点反项画成一个独立顶点, 并不是因为它在代数上不能与普通的$\phi^4$顶点合并. 按照本节的符号约定, 四次项可以直接写成
\begin{align}
    -\frac{\lambda}{4!}\phi_r^4-\frac{\delta_\lambda}{4!}\phi_r^4
    =-\frac{\lambda+\delta_\lambda}{4!}\phi_r^4,
\end{align}
所以也可以只引入一个顶点因子
\begin{align}
    -i(\lambda+\delta_\lambda)=-i\lambda-i\delta_\lambda.
\end{align}
因此, 把两者合并为一个整体顶点与把普通顶点$-i\lambda$和反项顶点$-i\delta_\lambda$分开, 在代数上完全等价.

之所以仍把反项画成独立顶点, 主要是为了清楚地追踪微扰阶数. 对四点函数的一圈精度而言,
\begin{align}
    \lambda=O(\lambda),\qquad \delta_\lambda=O(\lambda^2).
\end{align}
因此, 一个四点反项树图与一个由两个普通$\phi^4$顶点构成的一圈图都属于$O(\lambda^2)$, 必须在同一微扰阶中相加; 反项正是用来抵消该圈图的UV发散. 若一开始只保留合并后的顶点$-i(\lambda+\delta_\lambda)$, 计算时仍然需要按上式将它展开, 否则会掩盖各项所属的微扰阶, 也不便直接看出圈图的发散如何被反项抵消. 从这个意义上说, 将两类顶点分开只是重整化微扰论中更清楚的记账方式, 并没有改变理论.
...
\begin{align}
    i\mathcal M(s, t)=\begin{tikzpicture}[line width=1pt]
        \draw (-1.2, -0.7) -- (1.2, 0.7);
        \draw (-1.2, 0.7) -- (1.2, -0.7);
        \fill[white] (0,0) circle (0.3);
        \draw (0,0) circle (0.3);
...
\begin{align}
    i\mathcal M=\begin{tikzpicture}[line width=1pt]
        \draw (-1.2, -0.7) -- (1.2, 0.7);
        \draw (-1.2, 0.7) -- (1.2, -0.7);
        \fill[white] (0,0) circle (0.3);
        \draw (0,0) circle (0.3);
...
利用$i\mathcal M(s=4m^2, t=u=0)=-i\lambda$, 设$\Delta_0(c)=m^2-c_0x(1-x)-i\epsilon$, 其中$c=s, t, u$, 而$s_0=4m^2, t_0=u_0=0$, 我们可以解得反项
\begin{align}
...
从而有一阶修正后的四点关联函数
\begin{align}
    i\mathcal M&=-i\lambda+\frac{i\lambda^2}{32\pi^2}\sum_{c=s,t,u}\int_0^1\dx\left[\ln\frac{\Delta(c)+\Lambda^2}{\Delta_0(c)+\Lambda^2}\frac{\Delta_0(c)}{\Delta(c)}\right.\notag\\
    &\quad\left.-\frac{\Lambda^2}{\Lambda^2+\Delta(c)}+\frac{\Lambda^2}{\Lambda^2+\Delta_0(c)}\right].
\end{align}
可以发现, 现在无穷大就已经全部被吸收掉了, 取$\Lambda\to\infty$仍然可以保证关联函数是有限的. 于是我们直接计算$\Lambda\to\infty$后的极限, 得到
\begin{align}
    i\mathcal M&=-i\lambda-\frac{i\lambda^2}{32\pi^2}\sum_{c=s,t,u}\int_0^1\dx\ln\frac{\Delta(c)}{\Delta_0(c)}\notag\\
...
\begin{align}
    \int_0^1\dx\ln\left(m^2-cx(1-x)-i\epsilon\right)&=\ln m^2-2\notag\\
    &\quad+2\sqrt{\frac{4m^2}{c}-1-i\epsilon}\arctan\frac1{\sqrt{\frac{4m^2}{c}-1-i\epsilon}},
\end{align}
设辅助函数
\begin{align}
...
\begin{align}
    i\mathcal M&=-i\lambda-\frac{i\lambda^2}{16\pi^2}\left[A(s)+A(u)+A(t)-A(4m^2)-2A(0)\right].
\end{align}
而$A(4m^2)=0, A(0)=1$, 于是最终可得
\begin{align}
    \mathcal M(s, t, u)&=-\lambda-\frac{\lambda^2}{16\pi^2}\left[A(s)+A(u)+A(t)-2\right].
\end{align}
...
on-shell方案很清楚也很物理, 但是缺点在于随着圈数提高, 满足它的on-shell条件会越来越麻烦; 其次, 有些时候它计算出来的结果在特殊情况下会有奇异性. 比如上一节计算出来的$\phi^4$一阶修正中, 取$m\to0$就会导致奇异性, 这就是红外发散. 为了能够正确处理无质量粒子, 并且简化计算, 我们实际更常用的是MS或者$\overline{MS}$方案. 所谓的MS其实是Minimal Subtraction的缩写.
...
\begin{definition}[$\overline{MS}$方案]
    $\overline{MS}$方案就是指通过维度调控计算圈图, 反项抵消极点$\frac1\epsilon$与无关常数$\gamma, \ln4\pi$等.
\end{definition}
...
\begin{align}
    ...
    & i\mathcal M=-i\lambda-\frac{i\lambda^2}{32\pi^2}\sum_{c=s,t,u}\int_0^1\dx\ln\frac{m^2-cx(1-x)-i\epsilon}{\mu^2}.
\end{align}
...
散射振幅为
\begin{align}
    \mathcal M(s, t, u)=-\lambda-\frac{\lambda^2}{16\pi^2}\left[A(s)+A(t)+A(u)-3+\frac32\ln\frac{m^2}{\mu^2}\right].
\end{align}
...
于是
\begin{align}
    -iM^2(p^2)&=-\frac{ig^2}{16\pi^2}\left(\sqrt{\frac{4m^2}{p^2}-1-i\epsilon}\arctan\frac1{\sqrt{\frac{4m^2}{p^2}-1-i\epsilon}}-\frac12\right)\notag\\
...
\begin{align}
    ...
    &=-\frac{ig^3}{16\pi^2}\int\dx_1\dx_2\frac1{m^2-p^2x_1(1-x_1)-k^2x_2(1-x_2)-2x_1x_2p\cdot k-i\epsilon}.
\end{align}
显然也有反项为$0$
\begin{align}
    \delta_g=0.
...
\section{旋量QED的重整化}
QED的重整化是量子场论最伟大的成就之一, 正是因为其对电子反常磁矩的精确计算, 才使得人们开始接受量子场论的正确性. 在附录\ref{dirac-to-schrodinger}的推导中, 我们知道, 电子磁矩$\vec\mu_B=\frac{e\vec\sigma}{2m}$是贡献给哈密顿量的. 而精确的光谱学实验观测告诉我们, 实际上电子磁矩并不是精确地为这个值, 实际上为
\begin{align}
    \vec\mu_B=\frac g2\frac{e\vec\sigma}{2m},
\end{align}
其中$g$为修正因子,
\begin{align}
    g=2+\frac{e^2}{4\pi^2}+\cdots\approx2.0023228.
\end{align}
在后面我们会看到电子反常磁矩的来源正是量子场论对电子光子相互作用顶点的一圈修正.

我们首先引入反项
\begin{align}
    \mathcal L&=-\frac14F_r^2+\bar\psi_r(i\slashed\partial-m)\psi_r-e_r\bar\psi_r\slashed A_r\psi_r\notag\\
    &\quad-\frac14\delta_A F_r^2+\delta_\psi\bar\psi_r(i\slashed\partial)\psi_r-\delta_m\bar\psi_r\psi_r-\delta_e\bar\psi_r\slashed A_r\psi_r.
\end{align}
得到反项顶点
\begin{align}
    % 第一行：光子自能计数项
...
然后我们考虑计算电子反常磁矩. 电子反常磁矩的确定主要来源于氢原子光谱的测定, 因此我们也想要一个半经典的模型来处理一圈修正电子磁矩. 因为氢原子轨道上的电子是on-shell的, 因此我们可以计算代入on-shell条件与$\bar u(k), u(p)$缩并的散射振幅, 利用\eqref{ano-mu-cs}中的结果, 来分析得到电子反常磁矩. 具体来说, 设
\begin{align}
    \bar u(k)\begin{tikzpicture}[line width=1pt, baseline=(current bounding box.center)]
        % 费米子线（左上进，左下出）
        \draw[->] (-0.9, 0.7) -- (-0.45, 0.35); \draw (-0.45, 0.35) -- (0,0);
        \draw[->] (0, 0) -- (-0.45, -0.35);     \draw (-0.45, -0.35) -- (-0.9, -0.7);
        % 光子线（向右水平延伸）
        ...
        \draw (0,0) circle (0.3);
        \path[pattern=north west lines] (0,0) circle (0.3);
    \end{tikzpicture} u(p)&=i\bar u\Gamma^\mu u.
\end{align}
$\Gamma^\mu$只能是$\gamma^\mu$, $(p+k)^\mu$与$(p-k)^\nu$的线性组合. 因为Ward恒等式要求$(p-k)_\mu\mathcal M^\mu=0$, 因此结果中不应存在正比$(p-k)_\nu$的项, 于是只能是$\gamma^\mu, (p+k)^\mu$的线性组合. 而利用Gordon恒等式, 我们可以将$(p+k)^\mu$变为$\gamma^\mu, \sigma^{\mu\nu}(p-k)_\nu$的线性组合, 从而根据$\sigma^{\mu\nu}$前的系数得到反常磁矩.
...
\begin{align}
    &-\frac{ie^2}{2\pi^2}\left(p^2g^{\mu\nu}-p^\mu p^\nu\right)\int_0^1\dx x(1-x)\notag\\
    &\quad\times\left(\frac1\epsilon+\ln4\pi-\gamma-\ln\frac{m^2-p^2x(1-x)-i\epsilon}{\mu^2}\right)\notag
\end{align}
从而得到反项
\begin{align}
...
\begin{align}
    ...
    & \delta_m=-\frac{me^2}{4\pi^2}\left(\frac1\epsilon+\ln4\pi-\gamma\right),
\end{align}
因此
\begin{align}
...
\begin{align}
    ...
    &=-\frac{ie^3}{8\pi^2}\int\dx_1\dx_2\left[\left(\frac1\epsilon-\gamma+\ln4\pi-2-\ln\frac\Delta{\mu^2}\right)\gamma^\mu-\frac{A^\mu}\Delta\right].
\end{align}
其中
\begin{align}
    \Delta&=m^2(x_1+x_2)+2p\cdot k x_1x_2-p^2x_1(1-x_1)-k^2x_2(1-x_2)-i\epsilon,\\
    A^\mu&=-[\slashed p(1-x_1)-\slashed kx_2]\gamma^\mu[\slashed k(1-x_2)-\slashed px_1]-m^2\gamma^\mu\\\notag
    &\;\;\;+2m\left[p(1-2x_1)+k(1-2x_2)\right]^\mu.
\end{align}
不难得到反项
\begin{align}
    \delta_e=-\frac{e^3}{16\pi^2}\left(\frac1\epsilon-\gamma+\ln4\pi\right),
\end{align}
于是
\begin{align}
    \begin{tikzpicture}[line width=1pt, baseline=(current bounding box.center)]
        ...
        \draw (0,0) circle (0.3);
        \path[pattern=north west lines] (0,0) circle (0.3);
    \end{tikzpicture}&=-ie\gamma^\mu+\frac{ie^3}{8\pi^2}\int\dx_1\dx_2\left[\left(2+\ln\frac\Delta{\mu^2}\right)\gamma^\mu+\frac{A^\mu}\Delta\right]
\end{align}

利用这个顶点修正, 我们可以开始计算电子反常磁矩. 首先代入on-shell条件, $p^2=k^2=m^2$. 并且因为我们考虑的是电子绕氢原子的运动, 我们应当取长波极限, $(p-k)^\mu\to0$, 所以也认为$p\cdot k=m^2$. 于是
\begin{align}
    \Delta=m^2(x_1+x_2)^2.
\end{align}

根据前面的分析, 我们知道重点在于提取$(p+k)^\mu$前面的系数, 因此我们只算$A^\mu$中$p^\mu$与$k^\mu$的系数. 不难得到
\begin{align}
    A^\mu=2m(x_2-x_1x_2-x_1^2)p^\mu+2m(x_1-x_1x_2-x_2^2)k^\mu+\cdots.
\end{align}
利用$\Delta$在$x_1,x_2$互换的操作中不变的性质, 我们可以将$p^\mu, k^\mu$也分解为$x_1,x_2$互换的变换中奇偶变换的两部分, 也就是
\begin{align}
    x_2-x_1x_2-x_1^2=\frac{x_2-x_1x_2-x_1^2+x_1-x_1x_2-x_2^2}2+\frac{x_2-x_1x_2-x_1^-x_1+x_1x_2+x_2^2}2.
\end{align}
从而得到$(p+k)^\mu$前的系数为$(x_1+x_2)(1-x_1-x_2)$. 根据
\begin{align}
    \int\dx_1\dx_2\frac{(x_1+x_2)(1-x_1-x_2)}{(x_1+x_2)^2}=\frac12,
\end{align}
我们得到
\begin{align}
    \begin{tikzpicture}[line width=1pt, baseline=(current bounding box.center)]
        ...
        \draw (0,0) circle (0.3);
        \path[pattern=north west lines] (0,0) circle (0.3);
    \end{tikzpicture}&=-\frac e{2m}\bar u\sigma^{\mu\nu}(k-p)_\nu u+\frac{ie^3}{16\pi^2m}i\bar u\sigma^{\mu\nu}(k-p)_\nu u+\cdots,
\end{align}
于是不难得到
\begin{align}
    \frac g2=1+\frac{e^2}{8\pi^2}=1+\frac\alpha{2\pi}.
\end{align}
这正是Schwinger在1948年得到的著名结果.

\newpage

\section{重整化群结构}
\subsection{Callan-Symanzik方程}
以$\phi^4$理论为例, 我们定义$\beta$函数
\begin{align}
    \de{\lambda}{\ln\mu}=\beta(m^2, \lambda),
\end{align}
质量反常维度(anomalous dimension)
\begin{align}
    \de{\ln m^2}{\ln\mu}=\gamma_m(m^2,\lambda),
\end{align}
场反常维度
\begin{align}
    \frac12\de{\ln Z}{\ln\mu}=\gamma_\phi(m^2,\lambda).
\end{align}
在下一节我们会看到, 为什么说$\gamma_\phi$会被称作反常维度.

当我们得到重整化群方程后, 即可直接计算n点关联函数. 我们知道
\begin{align}
    \braket{\phi_{bare}\cdots}=Z_\phi^{\frac n2}\braket{\phi\cdots}=Z_\phi^{\frac n2}G^{(n)},
\end{align}
裸的$\phi_{bare}$是直接出现在Lagrangian中的, 与$\mu$的选取是无关的，因此
\begin{align}
    0=\pa{}{\ln\mu}\braket{\phi_{bare}\cdots}=\pa{}{\ln\mu}Z_\phi^{\frac n2}\braket{\phi\cdots}=Z_\phi^{\frac n2}G^{(n)}.
\end{align}
于是即可得到
\begin{theorem}[Callan-Symanzik方程]
    \begin{align}
        \left(\pa{}{\ln\mu}+n\gamma_\phi+\beta\pa{}{\lambda}+\gamma_m\pa{}{\ln m^2}\right)G^{(n)}=0.
    \end{align}
\end{theorem}

通过求解这个方程, 我们即可直接得到n点关联函数. 不过对于一般的情况, 计算可能比较复杂, 而且不会涌现出什么额外的对称性. 在下一节中我们会考虑在重整化群流中特殊的点上关联函数的特殊性质.

\subsection{不动点与反常维度}
\begin{definition}[不动点]
    在重整化群流中, 所有参量的$\beta$(不包括波函数重整化$Z$的)函数为$0$的点, 称为不动点.
\end{definition}

在不动点处, $\lambda, m^2$都变得与能标$\mu$无关了, 从而有
\begin{align}
    \gamma_\phi\left(m^2(\mu),\lambda(\mu)\right)=\gamma_\phi^*.
\end{align}
对于两点关联函数, Callan-Symanzik方程被简化为
\begin{align}
    \left(\pa{}{\ln\mu}+2\gamma_\phi^*\right)G^{(2)}=0.
\end{align}

对于无质量的标量粒子, $G^{(2)}$可以被写为
\begin{align}
    G^{(2)}(p)&=\frac{h(p)}{p^2}, 
\end{align}
其中$h$是一个无量纲的因子. 又因为粒子是无质量的, 若$h$与$p$有关, 就必须先通过唯一具有动量量纲的能标$\mu$将$p$无量纲化, 也就是要求
\begin{align}
    G^{(2)}(p)&=\frac{h(p/\mu)}{p^2}.
\end{align}
代入Callan-Symanzik方程我们得到
\begin{align}
    h'=2\gamma_\phi^*h/x,
\end{align}
也就是
\begin{align}
    h(\frac p\mu)=\left(\frac p\mu\right)^{2\gamma_\phi^*}.
\end{align}
因此我们得到
\begin{align}
    G^{(2)}(p)\propto\frac1{p^{2-2\gamma_\phi^*}}.
\end{align}
由此可见, 对于无质量场, 在不动点处的关联函数具有非常简单的幂律形式, 并且幂与经典场并不一样, 存在量子场论圈图修正所特有的$\gamma_\phi^*$修正. 这也是为什么$\gamma_\phi^*$被称作反常维度. 如果我们将其Fourier变换到位置空间, 则有
\begin{align}
    G^{(2)}(x)\propto\frac1{x^{d-2+2\gamma_\phi^*}}.
\end{align}
若定义$\Delta_\phi=\frac{d-2}2+\gamma_\phi^*$, 则我们得到
\begin{align}
    G^{(2)}(x)\propto\frac1{x^{2\Delta_\phi}}.
\end{align}
我们以后将在CFT中见到类似这样的完全相似的结构, 并且通过对称性的方式, 不计算圈图, 非微扰地计算得到这一结论.
