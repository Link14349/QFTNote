...
\subsubsection{Clifford代数\label{clifford}}
...
\subsubsection{Dirac旋量的二次量子化\label{2ndq-dirac}}
...
\begin{theorem}
    \begin{equation}
        \bar u^s\gamma^\mu u_{s'}=2p^\mu\delta_{ss'}
    \end{equation}
    \begin{equation}
        \bar v^s\gamma^\mu v_{s'}=2p^\mu\delta_{ss'}
    \end{equation}
\end{theorem}
\begin{proof}
    根据Dirac方程
    ...
    \end{equation}
    \begin{equation}
        \bar u^s\slashed p=m\bar u^s
    \end{equation}

    于是
    \begin{align}
        2m\bar u^s\gamma^\mu u_{s'}=\bar u^s\slashed p\gamma^\mu u_{s'}+\bar u^s\gamma^\mu\slashed pu_{s'}=p_\nu\bar u^s\{\gamma^\mu, \gamma^\nu\}u_{s'}=2p^\mu\cdot 2m\delta_{ss'}=4mp^\mu\delta_{ss'}
    \end{align}
    ...
    \end{equation}
\end{proof}
\kaishu 注: 但需要注意$\bar u^s\gamma^\mu v_{s'}\neq0$

因为考虑
\begin{equation}
    2m\bar u^s\gamma^\mu v_{s'}=\bar u^s\slashed p\gamma^\mu v_{s'}-\bar u^s\gamma^\mu\slashed pv_{s'}=\bar u^sp_\nu[\gamma^\nu, \gamma^\mu]v_{s'}\neq0
\end{equation}
\songti

接下来由于涉及到对易子的问题, 为了能够区分对易子的乘法顺序以及做乘法的方式(内积还是外积), 我们对$\gamma, \psi$引入指标:
\begin{align}
    &\psi\to\psi_A\\
    &\bar\psi\to\bar\psi^A\\
...
\subsection{旋量QED}
然后我们尝试和电磁场耦合. 同样考虑局部$U(1)$规范变换, 即
\begin{equation}
