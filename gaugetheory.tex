...
然后有对易子
\begin{align}
    [A^i_{\vec x}, E^j_{\vec y}]&=i\ddd p\exp{i\vec p\cdot(\vec x-\vec y)}\sum_r\epsilon_r^i(\vec p)\epsilon^j_r(\vec p)\\
    &=i\ddd p\exp{i\vec p\cdot(\vec x-\vec y)}(\delta^{ij}-\frac{p^ip^j}{\vec p^2})\\
