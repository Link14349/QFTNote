...
\begin{theorem}
    \begin{align}
        \int\d^nx\exp{-\frac12x^TAx+x^T J}=\sqrt{\frac{\dpi n}{\det A}}\exp{\frac12 J^T AJ}
    \end{align}
\end{theorem}
\subsection{$\Gamma$函数}
...
\begin{theorem}[Feynman积分公式]\label{feynman-trick}
    \begin{align}
        \frac1{A_1A_2\cdots A_n}=\int_0^1\dx_1\dx_2\cdots\dx_n\delta\left(\sum x_i-1\right)\frac1{\left[A_1x_1+A_2x_2+\cdots A_nx_n\right]^n}.
    \end{align}
\end{theorem}
更常用的是$n=2$的情况
\begin{align}
