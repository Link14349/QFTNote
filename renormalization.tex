...
\end{align}
其中后面的项被称为Counterterm. 通过恰当地选取Counterterm的值, 我们可以吸收掉计算中的无穷大, 并与实验结果正确地相匹配.

当然, 与真实物理量的匹配并不只这一种方式, 我们后面会看到, 还有其他将物理量与模型相匹配的方式, 不同的匹配方式都是一种Scheme.
\begin{definition}[Scheme]
    Scheme就是指将实验观测的物理量与物理模型匹配的方式.
\end{definition}
我们的微扰计算结果是强烈依赖于Scheme的, 理论上来说, 只有我们考虑无穷阶的微扰, 才能使得计算结果与Scheme无关(但是这是做不到的).

\begin{definition}[On-Shell Scheme]
    我们要求, 在on-shell scheme中, 按如下方式匹配counterterm:
    \begin{itemize}
...
我们以$\phi^4$理论与On-Shell Scheme为例, 介绍重整化的一般方法. 首先我们引入Counterterm, 得到
\begin{align}
    \mathcal L&=\frac12(\partial\phi_r)^2-\frac12 m^2\phi_r^2-\frac\lambda{4!}\phi_r^4+\frac12\delta_Z(\partial\phi_r)^2-\frac12\delta_m\phi^2-\frac{\delta_\lambda}{4!}\phi_r^4,
\end{align}
其中我们认为Counterterm都是微扰项, 并利用分部积分$\frac12\delta_Z(\partial\phi_r)^2=-\frac12\delta_Z\phi_r\Box\phi_r$, 可以得到Counterterm的顶点
...
根据如上计算结果可以进行物理量的匹配. 在On-Shell Scheme中, 极点位置为$m^2$要求
\begin{align}
...
\begin{align}
    iM(4m^2, 0)=-i\lambda.
\end{align}
这样我们就得到了三个方程, 而我们也恰有三个Counterterm$\delta_Z, \delta_m, \delta_\lambda$, 从而可以完全确定这三个Counterterm, 将发散的物理量都吸收到这三者中, 实现物理量的正确匹配.

\subsubsection{利用动量截断计算圈图}
上面的介绍可能略带抽象, 因此我们给出具体的计算例子来解释如何正确地匹配Counterterm实现物理量的重匹配.
...
\begin{align}
    \vcenter{\hbox{
        ...
        \begin{tikzpicture}[line width=1pt, scale=0.7, baseline=-0.1cm]
            ...
            \draw (-0.14,0.14) -- (0.14,-0.14);
        \end{tikzpicture}
    }}&=-i\lambda\int\dddd p\frac i{p^2-m^2+i\epsilon}+i(p^2\delta_Z-\delta_m).
\end{align}
...
从而
\begin{align}
    -i\lambda\int\dddd p\frac i{p^2-m^2+i\epsilon}=-i\lambda\int i\dddd{p_E}\frac{i}{-p_E^2-m^2+i\epsilon}.
\end{align}
由此可见, 我们成功将双曲空间中的积分化为了欧式空间上的积分.

利用$n$维球面角公式, 我们不难计算得到
\begin{align}
    -i\lambda\int i\dddd{p_E}\frac{i}{-p_E^2-m^2+i\epsilon}&=-\frac{i\lambda}{16\pi^2}\int_0^{+\infty}\frac{2p_E^3\d p_E}{p_E^2+m^2},
\end{align}
我们引入动量截断$\Lambda$, 将积分上限变为$\Lambda$, 则可计算得到
\begin{align}
    -i\lambda\int i\dddd{p_E}\frac{i}{-p_E^2-m^2+i\epsilon}&=-\frac{i\lambda}{16\pi^2}\left[\Lambda^2-\ln\left(1+\frac{\Lambda^2}{m^2}\right)\right].
\end{align}

于是可以得到传播子一阶修正
\begin{align}
    M^2(p^2)=\frac\lambda{16\pi^2}\left[\Lambda^2-\ln\left(1+\Lambda^2/m^2\right)\right]-p^2\delta_Z+\delta_m.
\end{align}
利用On-Shell Scheme的物理量匹配条件, 
\begin{align}
    &M^2(p^2)\Big|_{p^2=m^2}=0,\notag\\
    &\de{}{(p^2)}M^2(p^2)\Big|_{p^2=m^2}=0.\notag
\end{align}
我们解得
\begin{align}
    \delta_Z=0, \delta_m=-\frac\lambda{16\pi^2}\left[\Lambda^2-\ln\left(1+\Lambda^2/m^2\right)\right].
\end{align}
于是无穷大被完全抵消, 得到
\begin{align}
    M^2(p^2)=0.
...
\begin{align}
    ...
    \end{tikzpicture}
    % --- 中间的等号 ---
    &=\frac{(-i\lambda)^2}{2} \int \frac{d^4k}{(2\pi)^4} \frac{i}{k^2 - m^2 + i\epsilon} \frac{i}{(p-k)^2 - m^2 + i\epsilon}.
\end{align}
...
\begin{align}
    &=\frac{\lambda^2}2\int\int_0^1\dx\dddd k\frac1{\left[(k-px)^2+p^2x(1-x)-m^2+i\epsilon\right]^2}\notag\\
    &=\frac{\lambda^2}2\int\int_0^1\dx\dddd k\frac1{\left[k^2+p^2x(1-x)-m^2+i\epsilon\right]^2}, 
\end{align}
做Wick rotation可以得到
\begin{align}
    &=\frac{i\lambda^2}2\int\int_0^1\dx\dddd{k_E}\frac1{\left[-k^2+p^2x(1-x)-m^2+i\epsilon\right]^2}
\end{align}
...
利用$iM(s=4m^2, t=u=0)=-i\lambda$, 设$\Delta_0(c)=m^2-c_0x(1-x)-i\epsilon$, 其中$c=s, t, u$, 而$s_0=4m^2, t_0=u_0=0$, 我们可以解得Counterterm
\begin{align}
...
\begin{align}
    ...
    &=-i\lambda-\frac{i\lambda^2}{32\pi^2}\int_0^1\dx\ln\frac{m^2-cx(1-x)-i\epsilon}{m^2-c_0x(1-x)-i\epsilon}.
\end{align}
利用分部积分, 经过艰苦卓绝的爆算我们可以得到
\begin{align}
    \int_0^1\dx\ln\left(m^2-cx(1-x)-i\epsilon\right)&=-2+2\sqrt{\frac{4m^2}{c}-1-i\epsilon}\arctan\frac1{\sqrt{\frac{4m^2}{c}-1-i\epsilon}},
\end{align}
设辅助函数
\begin{align}
...
\subsubsection{利用维数计算圈图}

\subsubsection{什么是不可重整化?}
对于$\Delta_i\geq0$, 积分产生无穷大的根源是我们没有正确地匹配物理量与观测结果: 只要我们正确地将实验观测量与物理量进行匹配, 即可得到具有预测能力的物理理论. 但是对于$\Delta_i<0$的理论, 情况就糟糕很多了: 我们有无穷多个振幅(子图)是发散的, 这意味着我们需要引入无穷多个counterterm, 并与无穷多个实验测量结果进行拼配: 这使得我们的理论完全失去了预言能力. 读者可能会疑惑, 我们不是总共就这么几个参数吗, 为什么需要引入无穷多个counterterm. 这里我们做进一步的讨论, 考虑一个$\phi^6$理论
\begin{align}
...
\end{align}
这看起来没啥问题, 如果使用on-shell scheme, 我们只需要继续让$\delta_Z, \delta_m, \delta_\lambda$与实验观测结果匹配即可. 但是问题在于, 随着我们考虑更多顶点, 会出现我们原有Counterterm无法消除的无穷大, 这需要我们凭空在Lagrangian里面引入新的Counterterm来抵消无穷大, 最终引入无穷多个Counterterm以至于理论完全失去预言能力.
