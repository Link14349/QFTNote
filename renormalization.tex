...
\begin{align}
    ...
    \end{tikzpicture}
    &=(-i\lambda)^3\mu^{2\epsilon}\int \frac{\d^d l}{(2\pi)^d}\frac{i}{l^2 - m^2 + i\epsilon} \frac{i}{(p+l)^2 - m^2 + i\epsilon} \frac{i}{(k-l)^2 - m^2 + i\epsilon}\\
    &=-\frac{i\lambda^3}{32\pi^2}\int\dx_1\dx_2\frac1{m^2-p^2x_1(1-x_1)-k^2x_2(1-x_2)-2x_1x_2p\cdot k-i\epsilon}.
\end{align}
显然也有反项为$0$
\begin{align}
    \delta_g=0.
...
我们首先引入反项
\begin{align}
    \mathcal L=-\frac14F_r^2+\bar\psi_r(i\slashed\partial-m)\psi_r-e\bar\psi\slashed A\psi-\frac14\delta_A F^2+\delta_\psi\bar\psi_r(i\slashed\partial)\psi_r-\delta_m\bar\psi_r\psi-\delta_e\bar\psi_r\slashed A\psi_r.
\end{align}
得到反项顶点
\begin{align}
    % 第一行：光子自能计数项
...
\end{align}
因为我们知道最后的结果$i\Pi^{\mu\nu}(p)\propto i(p^2g^{\mu\nu}-p^\mu p^\mu)$, 因此我们可以直接只计算以上积分中正比$g^{\mu\nu}$的部分, 直接得到结果. 
\begin{align}
