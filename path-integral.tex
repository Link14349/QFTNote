...
\begin{align}
    ...
    &=VT\cdot\int\ddd p\int\frac{\d\omega}{2\pi}\ln(-\omega^2+\om{k}^2-i\epsilon)\\
    &=VT\cdot\int\ddd p\pa{}{\alpha}\int\frac{\d\omega}{2\pi}\frac{-1}{(-\omega^2+\om{k}^2-i\epsilon)^\alpha}
\end{align}
利用Mathematica进行艰苦卓绝的计算, 我们得到
\begin{align}
    \Tr\ln(\Box+m^2)=VT\int\ddd p\sqrt{-\om k^2+i\epsilon}=VT\int\ddd p i\om k.
\end{align}
又根据
\begin{align}
...
于是我们有
\begin{align}
    \frac{E_0}{V}=\int\ddd p\frac12\om k.
\end{align}
这和正则量子化的结果一致.
