...
动量截断非常物理也非常直接, 在处理$\phi^4$场论的时候非常好用, 但是涉及到规范场则会显得有些力不从心: 一个强行的动量截断显然破坏了Lorentz对称性与规范对称性. 一个优雅的解决方案是使用维数调控: 假设我们并不是在$d=4$维考虑量子场论, 而是在$d=4-2\epsilon$考虑, 这样计算出的结果在$2\epsilon$非整数的时候是收敛的, 而在$\epsilon=0$处有一个极点, 也就是将无穷大的发散都体现在$\frac1\epsilon$中. 维数调控的物理意义相对不那么清晰, 但是在计算上会更加方便简单.
...
\end{align}
这里新引入了一个动量量纲的中间因子$\mu$, 是因为$\d^4 p$变成了$\d^{4-2\epsilon}p$, 缺少了$2\epsilon$次的动量量纲, 我们需要手动补上一个有动量量纲的常数$\mu$来保持量纲.
