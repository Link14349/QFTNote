...
在\ref{sr-lorentz}节中, 我们已经讨论了作用在时空坐标上的Lorentz变换. 接下来我们想要把Lorentz变换的操作抽象化, 一般化, 将其提升到群表示论的高度, 于是我们有一个用抽象的Lorentz变换参数$\omega_{\mu\nu}$表示的对某一个对象进行的Lorentz变换, 这成为Lorentz群的一个表示.
...
这里$\gamma^\mu$矩阵的形式被称为手征表象.
\begin{definition}[手征表象]
    \begin{align}
            \gamma^\mu=\begin{pmatrix}
            0 & \sigma^\mu\\
            \bar\sigma^\mu & 0
        \end{pmatrix},
    \end{align}
    其中$\sigma^\mu=(\mathds{1}, \sigma^i), \bar\sigma^\mu=(\mathds{1}, -\sigma^i)$, $\sigma^i$为Pauli矩阵.
\end{definition}
我们做一个幺正变换也可以切换到其他表象中, 比如取变换
\begin{align}
    U=\frac1{\sqrt2}\begin{pmatrix}
        \mathds1 & -\mathds{1}\\
        \mathds1 & \mathds1
    \end{pmatrix}, \gamma^\mu\to U^\dagger\gamma^\mu U,
\end{align}
就得到了更常用也更方便计算的Dirac表象.
\begin{definition}[Dirac表象]
    \begin{align}
...
\begin{equation}
    [\gamma^j, \gamma^k]=-[\sigma^j, \sigma^k]\begin{pmatrix}
        1 & 0\\
        0 & 1
    \end{pmatrix}=-2i\epsilon_{jkl}\sigma^l\begin{pmatrix}
        1 & 0\\
        0 & 1
    \end{pmatrix},
\end{equation}
即
\begin{equation}
    \epsilon_{ijk}[\gamma^j, \gamma^k]=-4i\sigma^i\begin{pmatrix}
        1 & 0\\
        0 & 1
    \end{pmatrix},
\end{equation}
...
于是
\begin{align}
    \delta\psi&=\frac i2\epsilon_{ijk}\theta^i(\frac i4[\gamma^j, \gamma^k])\psi+iv^i(\frac i4[\gamma^i, \gamma^0])\\
...
\begin{theorem}[Gordon恒等式]\label{ugammau-contraction}
...
\end{equation}
这样就可以得到正定的结果, 并且这同时要求了Fermion负的真空能:
\begin{align}
    \frac{E}{V}=\textcolor{red}{-}\sum_s\int\ddd p\om p.\label{canonical-fermion-vaccum-energy},
\end{align}
...
\begin{equation}
    \braket{\psi_A(x)\bar\psi^B(y)}=\int\dddd p\frac{i(\slashed p+m)}{p^2-m^2+i\epsilon}\exp{-ip(x-y)},
\end{equation}
不难看出这是协变的.
...
\subsection{旋量的路径积分}
对于旋量场, 我们同样可以通过路径积分的方式进行量子化. Naively我们可能认为, 就定义一个有4*2个自由度的路径积分即可
\begin{align}
    \int\mathcal D\psi\mathcal D\bar\psi\exp{iS}.
\end{align}
但这样有个问题, 我们计算关联函数
\begin{align}
    \braket{\bar\psi\psi}=\int\mathcal D\psi\mathcal D\bar\psi\bar\psi\psi\exp{iS},
\end{align}
等号的右边$\bar\psi, \psi$显然是对易的, 但是左边因为我们$\mathcal T$的定义, 是反对易的. 这显然有问题! 要解决这个矛盾, 我们只能想办法让右边的路径积分里$\bar\psi, \psi$反对易. 这似乎是非常奇怪的要求, 但我们看到, 数学家Grassmann早就给我们准备好了合适的数学工具: Grassmann数.
\subsubsection{Grassmann代数}
\kaishu 
    出于严谨性的考虑, 我们这里可以简单补充一下代数的定义:
    \begin{definition}[代数]
        考虑一个定义在域$\mathbb F$上的线性空间$V$, 若它同时是一个环, 且数乘与环乘法相容:
        \begin{align}
            \lambda(\vec u\vec v)=\vec u(\lambda\vec v),
        \end{align}
        则称$V$是一个(结合)代数.
    \end{definition}
\songti
\begin{definition}[Grassmann代数]
    Grassmann为定义在域$\mathbb C$上的非交换代数, 并且元素乘法满足反对易关系:
    \begin{align}
        \eta\theta=-\theta\eta.
    \end{align}
\end{definition}
...
\begin{align}
    \eta^2=0,
\end{align}
这意味着所有定义在Grassmann代数上的函数都可以写为一次函数:
\begin{align}
    f(\eta)=\theta+B\eta.
\end{align}
\begin{example}
...
然后我们考虑Grassmann函数的导数, 定义
\begin{definition}[Grassmann函数的导数]
    \begin{align}
        \de{}{\eta}(\theta+B\eta)=B.
    \end{align}
\end{definition}
...
\begin{align}
    &\pa{}{\eta}(\eta\theta)=\theta, \\
    &\pa{}{\theta}(\eta\theta)=\pa{}{\theta}(-\theta\eta)=\eta.
\end{align}

然后我们尝试定义Grassmann函数的积分. 因为我们定义积分之后想要将之利用到路径积分上去, 所以积分测度不变是很重要的性质, 也就是:
\begin{align}
    \int\d\eta f(\eta)=\int\d(\eta+\alpha) f(\eta).
\end{align}
对于$f(\eta)=A+B\eta$, 这要求
\begin{align}
    \int\d\eta (\theta+B\eta)=\int\d\eta \theta+B(\eta-\alpha).
\end{align}
因此我们得知, 对于常Grassmann数的积分
\begin{align}
    \int\d\eta\alpha=0,
\end{align}
...
\end{align}
于是我们有
\begin{definition}[Grassmann函数的积分]
    \begin{align}
        \int\d\eta(\theta+B\eta)=B.
    \end{align}
\end{definition}

我们在复域上考虑Grassmann代数, 可以将之分解为
\begin{align}
    \Psi=\psi_1+i\psi_2, \Psi^\dagger=\psi_1-i\psi_2,
\end{align}
从而认为$\Psi, \Psi^\dagger$为基本自由度.

然后我们考虑Grasmann代数的Gaussian积分. 首先是单变量的情况
\begin{align}
    \int\d\theta^*\d\theta\exp{-\theta^*a\theta}=a,
\end{align}
不难证明
\begin{align}
    \int\d\theta^*_n\d\theta_n\cdots\int\d\theta^*_1\d\theta_1\exp{-\vec\theta^*A\vec\theta}=|A|,
\end{align}
类似地, 我们可以证明
\begin{align}
    \int\d\theta^*_n\d\theta_n\cdots\int\d\theta^*_1\d\theta_1\theta_i\theta_j^*\exp{-\vec\theta^*A\vec\theta}=|A|(A^{-1})_{ij}.
\end{align}
这个只需要注意到, $A$的代数余子式$A_{ji}=|A|(A^{-1})_{ij}$即可证明.

\subsubsection{Dirac旋量的路径积分}
我们可以将$\theta^*$都乘一个$\gamma^0$得到$\bar\theta$, 再做路径积分也是等价的, 于是我们直接定义
\begin{align}
    Z[J]=\int\mathcal D\bar\psi\mathcal D\psi\exp{i\int\d^4x\bar\psi(i\slashed\partial-m)\psi+\bar J\psi+\bar\psi J},
\end{align}
不难得到
\begin{align}
    Z[J]=\exp{-\int\d^4x\d^4y\bar J(x)S_F(x-y)J(y)},
\end{align}
其中
\begin{align}
    S_F(x)=\int\dddd p\frac{i(\slashed p+m)}{p^2-m^2+i\epsilon}\exp{-ipx}.
\end{align}
并且有
\begin{align}
    \braket{\psi_1\bar\psi_2\cdots}=\frac1{Z[0]}\left(-i\frac{\delta}{\delta\bar J}\right)\left(i\frac{\delta}{\delta J}\right)\cdots Z[J]\Big|_{J=0}.
\end{align}
...
\begin{align}
    \braket{\psi(x)\bar\psi(y)}=S_F(x-y)=\int\dddd p\frac{i(\slashed p+m)}{p^2-m^2+i\epsilon}\exp{-ip(x-y)}.
\end{align}

\begin{example}
    利用\ref{vac-energy-path-integral}节的结果, 我们同样可以计算Fermion的真空零点能, 并且验证它和\eqref{canonical-fermion-vaccum-energy}的结果是一致的. 计算配分函数
    \begin{align}
        Z[0]&=\int\mathcal D\psi\mathcal D\bar\psi\exp{i\int\d^4x\bar\psi(i\slashed\partial-m)\psi}=\det{(i\slashed\partial-m)}\notag\\
        &=\exp{\Tr{\ln(i\slashed\partial-m)}}\propto\exp{-iE_0T},
    \end{align}
    需要注意, 这里因为$\det$中整体的因子$-i$并不影响真空零点能的计算: 我们只要提取指数部分, 前面的$N$个$-i$的常数贡献我们并不关心. 而
    \begin{align}
        \Tr\ln(i\slashed\partial-m)&=VT\int\dddd p\Tr\ln(\slashed p-m)\\
        &=VT\int\dddd p\ln\det{(\slashed p-m)}.
    \end{align}
    而
    \begin{align}
        \slashed p-m=\begin{bmatrix}
            -m & p_\mu\sigma^\mu\\
    ...
    利用Schur补公式, 我们有
    \begin{align}
        \det{(\slashed p-m)}&=\det{(-m)}\det{\left[-m-p_\mu\bar\sigma^\mu\left(-\frac1m\right)p_\nu\sigma^\nu\right]}\\
        &=\det{(p_\mu p_\nu\bar\sigma^\mu\sigma^\nu-m^2)}\\
        &=\det{\left((\omega^2-m^2)-p_ip_j\sigma^i\sigma^j\right)}\\
        &=\det{\left((\omega^2-m^2)-\vec p^2\right)}\\
        &=\det{(p^2-m^2)}\\
        &=(p^2-m^2)^2.
    \end{align}
    接下来的计算与对Boson求零点能一样, 直接计算可得
    \begin{align}
        \Tr\ln(\i\slashed\partial-m)&=2VT\int\dddd p\ln(p^2-m^2)\\
        &=2VT\int\dddd p\ln(m^2-p^2+i\epsilon)\\
        &=2VT\int\ddd p i\om p.
    \end{align}
    于是我们通过路径积分得到真空零点能
    \begin{align}
        \frac{E_0}{V}=-2\int\ddd p\om p=-\sum_s\int\ddd p\om p,
    \end{align}
    这与我们正则量子化的计算结果一致.
\end{example}
...
\begin{equation}
    X\psi(x)X^\dagger=\brm X\psi(\Lambda_X\cdot x),
\end{equation}
其中$\brm X$是对场量的变换算符, $\Lambda_X$是对坐标的变换, 而$X$是一个幺正算符
\begin{equation}
    XX^\dagger=1.
\end{equation}
...
\begin{equation}
    C\psi(x)C^\dagger=\brm C\psi^\dagger(x).
\end{equation}
...
对于场算符, 我们要求$T$的表现形式为
\begin{equation}
    T^\dagger\psi(t, \vec x)T=\boldsymbol{\rm T}\phi(-t, \vec x).
\end{equation}

可以发现, 这是一个依赖于具体参考系的算符: 若做Lorentz变换, $(-t', \vec x')$所对应的时空点与$(-t, \vec x)$所对应的是不一样的.
...
\begin{equation}
    \mathcal L=-\frac14F^2+\bar\psi(i\slashed D-m)\psi,
\end{equation}
即
\begin{equation}
    \mathcal L=-\frac14F^2+\bar\psi(i\slashed \partial-m)\psi-eA_\mu\bar\psi\gamma^\mu\psi.
\end{equation}
...
\end{align}
如果我们再作用上$(i\slashed\partial-e\slashed A+m)$, 利用
\begin{align}
    &(i\slashed\partial-e\slashed A+m)((i\slashed\partial-e\slashed A-m))\notag\\
    =&(i\slashed\partial-e\slashed A)(i\slashed\partial-e\slashed A)-m^2\notag\\
    =&\frac12(i\partial_\mu-eA_\mu)(i\partial_\nu-eA_\nu)\{\gamma^\mu, \gamma^\nu\}+\frac12(i\partial_\mu-eA_\mu)(i\partial_\nu-eA_\nu)[\gamma^\mu, \gamma^\nu]-m^2,
\end{align}
并定义
\begin{align}
...
即得
\begin{align}
    \left((i\partial-eA)^2-\frac e2F_{\mu\nu}\sigma^{\mu\nu}-m^2\right)\psi=0.
\end{align}
可以发现, 这个方程与耦合电磁场的复标量场的EoM非常像, 只是多了一项$-\frac e2F_{\mu\nu}\sigma^{\mu\nu}$的贡献. 利用附录\ref{dirac-to-schrodinger}我们不难得到, 这正是磁矩的贡献. 一般地, 磁矩的贡献为
\begin{align}
    -g\frac e4F_{\mu\nu}\sigma^{\mu\nu},
\end{align}
在这里$g=2$. 在QED中顶点的一圈图会给予$g$新的修正.
...
\subsection{电子磁矩与散射截面的关系}
我们考虑电子与电磁场相互作用的顶点$-ie\gamma^\mu$, 如下一节的例\ref{rutherford}一样, 我们将原子核对电子的电磁场做经典处理, 可以直接计算电子在原子核作用下的散射截面. 在这里我们讨论自旋$\frac12$对这个散射截面带来的额外贡献: 磁矩. 利用定理\ref{ugammau-contraction}的Gordon恒等式, 我们有
\begin{align}
    \bar u(k)(-ie\gamma^\mu)u(p)=-\frac{ie}{2m}\left[(k+p)^\mu\bar u(k)u(p)-i\bar u(k)\sigma^{\mu\nu}(k-p)_\nu u(p)\right],
\end{align}
其中
\begin{align}
    \sigma^{\mu\nu}(k-p)=\frac i2[\gamma^\mu, \gamma^\nu]=2S^{\mu\nu}.
\end{align}
可以看到, 第一项的结构和标量QED的结构是完全一致的, 只有第二项才是$\frac12$自旋带来的独特贡献. 我们会看到, 这个正是磁矩的贡献. 我们计算散射振幅
\begin{align}
    i\mathcal M=-ie\bar u(k)\gamma^\mu u(p)A_\mu=-\frac{ie}{2m}\left[(k+p)^\mu\bar u(k)u(p)-i\bar u(k)\sigma^{\mu\nu}(k-p)_\nu u(p)\right]A_\mu,
\end{align}
利用\eqref{born-amplitude-V}, 我们知道, 自旋$\frac12$给振幅的额外贡献会贡献一个额外的势能
\begin{align}
    -\frac m{2\pi}\tilde V(\vec k-\vec p)2\pi\delta(\om k-\om p)=\frac{ie}{2m}\bar u(k)\sigma^{\mu\nu}(k-p)_\nu u(p)A_\mu\delta(\om k-\om p)\label{ano-mu-cs},
\end{align}
这里需要注意, 我们习惯的是位形空间中的三维势$V$, 到四维时空后, 会自动多一个$2\pi\delta$, 我们需要手动去除. 然后利用动量空间中的$(k-p)_\nu$对应于位形空间的$\partial_\nu$, 得到
\begin{align}
    \tilde V(\vec k-\vec p)=\frac{e}{4m^2}\bar u(k)\sigma^{\mu\nu}\tilde F_{\mu\nu}u(p).
\end{align}
利用, 
\begin{align}
    & \sigma^{ij}=\epsilon^{ijk}\Sigma^k, \Sigma^k=\begin{pmatrix}
        \sigma^k & 0\\
        0 & \sigma^k.
    \end{pmatrix}\\
    & \sigma^{0i}=i\begin{pmatrix}
        0 & \sigma^i\\
        \sigma^i & 0.
    \end{pmatrix}=i\gamma^0\gamma^i.
\end{align}
忽略电场, 我们得到
\begin{align}
    \tilde V(\vec k-\vec p)=-\frac{e}{4m}\bar u(k)\vec B\cdot\vec\Sigma u(p),
\end{align}
利用$\bar u(p)u(p)=2m$, 并且代入$u(p)$关于自旋$\xi$的解, 我们可知在负电子的自旋Hilbert空间中
\begin{align}
    \tilde V(\vec k-\vec p)=-\frac{e}{2m}\vec B\cdot\vec\sigma.
\end{align}
可见, 这正是磁矩的贡献! 如果在圈图修正后, 这项顶点变成了$-i\frac g2\bar u(k)\sigma^{\mu\nu}(k-p)_\nu u(p)$, $g\neq 2$, 就说明量子效应给予了电子反常磁矩.

\subsection{树图阶散射计算}
接下来我们开始计算几个简单的例子来熟悉QED的计算, 并获得它们的散射截面. 在计算之前, 我们首先需要考虑一个问题: 在实际实验中, 我们是并不知道粒子的自旋设置的, 或者说它们的自旋方向是完全随机的, 而我们的实验只能测量随机方向自旋的总效应: 对于入射态, 我们需要对不同自旋方向的$|\mathcal M|^2$取平均; 对于出射态, 因为不同的自旋出射都贡献到总的散射截面中, 因此我们需要直接求和. 比如考虑$n\to m$散射, 我们就有
\begin{equation}
...
\begin{example}[Bhabha散射]
    ...
    \begin{align}
        \left(\frac{\d\sigma}{\d\Omega}\right)_{CM}&=\frac{\alpha^2}{4E_{CM}|\vec p_i|}\left((\frac us+\frac ut)^2+\frac{t^2}{s^2}+\frac{s^2}{t^2}\right)\\
        &=\frac{\alpha^2}{8E^2}\left((\frac us+\frac ut)^2+\frac{t^2}{s^2}+\frac{s^2}{t^2}\right).
    \end{align}
    ...
    因此有最终结论
    \begin{equation}
        \left(\frac{\d\sigma}{\d\Omega}\right)_{CM}=\frac{\alpha^2}{8E^2}\left(\left(-\cos^2(\theta/2)+\cot^2(\theta/2)\right)^2+\sin^4(\theta/2)+\frac1{\sin^4(\theta/2)}\right)
    \end{equation}
...
\begin{example}[Møller散射]
    ...
    然后计算其模长平方
    \begin{align}
        |\mathcal M_{aver}|^2&=2e^4\left(\frac{(s-2m^2)^2+(t-2m^2)^2+4m^2u}{u^2}\right.\notag\\
        &\;\;\left.+\frac{(s-2m^2)^2+(u-2m^2)^2+4m^2t}{t^2}+2\frac{(s-2m^2)(s-6m^2)}{tu}\right)
    \end{align}
\end{example}
\begin{example}[Campton散射]
    ...
    \begin{align}
        ...
        &t=2m^2-2p_1p_3=-2p_2p_4\\
        &u=m^2-2p_1p_4=m^2-2p_2p_3
    \end{align}
    做代换, 经过艰苦卓绝的计算我们可以得到
    \begin{align}
        |\mathcal M_{aver}|^2&=2e^4\left[\frac{m^2-u}{s-m^2}+\frac{s-m^2}{m^2-u}\right.\notag\\
        &\quad\left.+\frac{4m^2t}{(s-m^2)(m^2-u)}+\frac{4m^4t^2}{(s-m^2)^2(m^2-u)^2}\right],
    \end{align}
    其中 $t=2m^2-s-u$.
    ...
    于是可以化简得到
    \begin{align}
        \de{\sigma}{\Omega}&=\frac{e^4}{32\pi^2m^2}\frac{\omega'^2}{\omega^2}\left(\frac{\omega'}{\omega}+\frac{\omega}{\omega'}-\sin^2\theta\right)
    \end{align}
    其中
    \begin{equation}
...
\end{example}
\begin{example}[Rutherford散射(半量子化)]\label{rutherford}
    ...
    \begin{align}
        ...
        &\;-ie\int\d^4xA_\mu\int\dddd p\frac{i(\slashed p+m)}{p^2-m^2}\exp{-ip(x_1-x)}\gamma^\mu\notag\\
        &\quad\times\int\dddd q\frac{i(\slashed q+m)}{q^2-m^2}\exp{-iq(x-x_2)}
    \end{align}
    利用旋量LSZ公式, 我们有
    \begin{align}
    ...
    \begin{equation}
        \braket{p'|i\mathcal T|p}=i\mathcal M\cdot2\pi\delta(E_i-E_f).
    \end{equation}
