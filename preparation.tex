\section{绪论}
这是我在上2025年秋季学期复旦大学物理系周洋老师开设的量子场论以及春季学期腾飞老师开设的规范场论课程产生的笔记, 主要记录了这学期我学习量子场论期间学会的所有内容以及思考过程. 我补充了一些周老师没有提到或者没有讲清楚的内容, 并融合了历次作业中部分有启发性的题目.
...
\begin{theorem}
    \begin{align}
        \int_a^b f(x) \delta(g(x)) \mathrm dx &= \sum_{i\in \{i~|~g(x_i)=0, x_i\in[a, b]\}}\frac{f(x_i)}{|g'(x_i)|}\notag\\
        &= \sum_{i\in \{i~|~g(x_i)=0\}}\frac{f(x_i)}{|g'(x_i)|}\Theta(b-x)\Theta(x-a)
    \end{align}
\end{theorem}
...
\begin{proof}
    \begin{align}
        (f, Ag)&=\eta(f)(Ag)=(A^*\circ\eta(f))(g)\notag\\
        &=\eta[(\eta^{-1}A^*\circ\eta)(f)](g)=\eta[(A^\dagger)(f)](g)=(A^\dagger f, g)
    \end{align}
    \begin{equation}
...
\begin{proof}
    ...
    \begin{align}
        S^\dagger(cf)&=\eta^{-1}\circ S^*\circ\eta(cf)=\eta^{-1}[S^*(c\eta(f))]=\eta^{-1}[c^*S^*(\eta(f))]\notag\\
        &=c^*\eta^{-1}[S^*(\eta(f))]=c^*S^\dagger(f)
    \end{align}
...
我们可以将反线性幺正算符$A$分解为一种特殊的反线性幺正算符$K$与另一个幺正算符$U$的组合, 即
\begin{equation}
    A=UK,
\end{equation}
其中, 反线性幺正算符$K$满足$K^2=1$.

\subsection{时间演化}
\begin{definition}[时间演化算子]
    量子态的演化可以用一个幺正的时间演化算子$U(t)$描述, 其满足
    \begin{align}
        \ket{\psi(t)}=U(t)\ket{\psi(0)}.
    \end{align}
\end{definition}
\begin{theorem}[Schrödinger方程]
    \begin{equation}
        i\frac{\partial}{\partial t}U(t)=HU(t)
    \end{equation}
\end{theorem}

我们有解
\begin{align}
    U(t)=\mathcal T\exp{\int-iH\d t}
\end{align}
...
设
\begin{align}
    a=\sqrt{\frac{m\omega}{2}}\left(x+\frac{i}{m\omega}p\right), \quad a^\dagger=\sqrt{\frac{m\omega}{2}}\left(x-\frac{i}{m\omega}p\right).
\end{align}
即
\begin{align}
    x=\frac{a+a^\dagger}{\sqrt{2m\omega}}, \quad p=-i\sqrt{\frac{m\omega}2}(a-a^\dagger).
\end{align}
...
设$N=a^\dagger a$, $N$是厄米算符, $N^\dagger=N$, 我们有
\begin{align}
...
\begin{definition}[连续态(Coherent State)]
    连续态$\ket\alpha$定义为$a$的本征矢
    \begin{equation}
...
不难验证, 
\begin{align}
    \ket\alpha=C\sum_n\frac{\alpha^n}{\sqrt{n!}}\ket n=C\sum_n\frac{\alpha^n}{n!}\sqrt{n!}\ket n=C\sum_n\frac{\alpha^n}{n!}(a^\dagger)^n\ket0,
\end{align}
其中$C$为归一化系数. 从而得到
\begin{theorem}
    \begin{align}
        \ket\alpha=\exp{-\frac{|\alpha|^2}2}\exp{\alpha a^\dagger}\ket 0.
    \end{align}
\end{theorem}

计算$x, p, x^2, p^2$的平均值, 我们有
\begin{align}
    &\braket{x}=\braket{\alpha|x|\alpha}=\sqrt{\frac2{m\omega}}\rm{Re}\alpha\\
    &\braket{x^2}=\braket{\alpha|x^2|\alpha}=\frac1{2m\omega}(1+4(\rm{Re}\alpha)^2)\\
    &\braket{p}=\braket{\alpha|p|\alpha}=\sqrt{2m\omega}\rm{Im}\alpha\\
    &\braket{p^2}=\braket{\alpha|p^2|\alpha}=\frac{m\omega}{2}(1+4(\rm{Im}\alpha)^2).
\end{align}
所以有
\begin{align}
    \Delta x^2=\braket{x^2}-\braket{x}^2=\frac1{2m\omega}, \quad\Delta p^2=\braket{p^2}-\braket{p}^2=\frac{m\omega}{2},
\end{align}
...
根据
\begin{align}
    \alpha\braket{x|\alpha}=\braket{x|a|\alpha}=\sqrt{\frac{m\omega}2}\braket{x|x+\frac i{m\omega}p|\alpha},
\end{align}
我们有微分方程
\begin{align}
    \pa{}x\braket{x|\alpha}=-m\omega\left(x-\sqrt{\frac 2{m\omega}}\alpha\right)\braket{x|\alpha},
\end{align}
于是可以解得波函数是Gaussian分布
\begin{align}
    \braket{x|\alpha}=\left(\frac{m\omega}\pi\right)^{\frac14}\exp{-\frac12m\omega(x-\sqrt{\frac 2{m\omega}}\alpha)^2},
\end{align}

考虑连续态的时间演化, 
\begin{align}
    \ket{\alpha(t)}=U(t)\ket{\alpha(0)}.
\end{align}
不难得到
\begin{align}
    \ket{\alpha(t)}=\exp{-i\frac\omega2 t}\ket{\alpha(0)\exp{-i\omega t}},
\end{align}

因此我们有
\begin{align}
    &\braket{x(t)}=\sqrt{\frac2{m\omega}}|\alpha|\cos(\rm{Arg}\alpha-\omega t)\\
    &\braket{p(t)}=\sqrt{2m\omega}|\alpha|\sin(\rm{Arg}\alpha-\omega t).
\end{align}

\subsection{单粒子散射}
本节介绍在量子力学框架内, 处理单粒子短程势场散射问题. 考虑单粒子静态Schr\"odinger方程,
\begin{align}
    (-\frac{\nabla^2}{2m}+V(\vec r))\psi(\vec r)=E\psi(\vec r).
\end{align}
我们设入射粒子是能量为$E=\frac{k^2}{2m}$的单色波$\exp{ikz}$, 也就是零级量$\psi_0=\exp{ikz}$. 我们可以通过Green函数做微扰求解这个定态Schr\"odinger方程. 我们将方程化为, 
\begin{align}
    (\frac{\nabla^2}{2m}+E)\psi=\epsilon V\psi,
\end{align}
其中$\epsilon$是一个小的微扰参量, 我们在最后会取$\epsilon\to1$. 考虑Green函数
\begin{align}
    (\frac{\nabla^2}{2m}+E)G(\vec x-\vec x')=\delta^3(\vec x-\vec x'),
\end{align}
不难得到
\begin{align}
    G(\vec x-\vec x')=2m\int\ddd p\frac{\exp{i\vec p\cdot(\vec x-\vec x')}}{k^2-p^2}=-\frac m{2\pi}\frac{\exp{ik|\vec x-\vec x'|}}{|\vec x-\vec x'|},
\end{align}
这是一个球面波. 然后不难得到定态波函数关于$\epsilon$的级数展开, 
\begin{align}
    \psi=\psi_0+\epsilon G*(V\psi_0)+\epsilon^2 G*[VG*(V\psi_0)]+\cdots.
\end{align}
其中, $*$表示卷积,
\begin{align}
    (G*V\psi)(\vec x)\equiv\int\d^3x'G(\vec x-\vec x')V(\vec x')\psi(\vec x').
\end{align}
我们还可以做多极展开, 因为我们最后计算散射截面, 只关心$|\vec x|\to\infty$处的粒子流, 因此展开到一阶就完全足够了.
\begin{align}
    G*V\psi_0(x)&=-\int\d^3x'\frac m{2\pi}\frac{\exp{ik|\vec x-\vec x'|}}{|\vec x-\vec x'|}V(\vec x')\psi_0(\vec x')\notag\\
    &\approx-\frac{m\exp{ik|\vec x|}}{2\pi|\vec x|}\int\d^3x'\exp{-ik\frac{\vec x'\cdot\vec x}{|\vec x|}}V(\vec x')\psi_0(\vec x').
\end{align}
于是
\begin{align}
    \psi(r)\approx\exp{ikz}-\frac{m\exp{ikr}}{2\pi r}\int\d^3x'\exp{-ik\frac{\vec x'\cdot\vec x}{|\vec x|}}V(\vec x')\psi_0(\vec x').
\end{align}
我们一般记
\begin{align}
    \psi(r)\approx\exp{ikz}+f(\theta,\varphi)\frac{\exp{ikr}}{r},
\end{align}
这里$f(\theta, \varphi)$被称为散射振幅, 这样计算散射的方式被称为Born近似. 在一阶近似下, 设$\vec q=k\frac{\vec x}{|\vec x|}$, 则不难发现这其实是个傅里叶变换
\begin{align}
    f(\vec q)=-\frac{m}{2\pi}\int\d^3x'\exp{-i\vec q\cdot\vec x'}V(\vec x')\psi_0(\vec x').
\end{align}
如果在动量空间中计算, 设$\tilde V, \tilde{\psi_0}$分别是$V, \psi_0$的傅里叶变换, 我们就直接有
\begin{align}
    f(\vec q)=-\frac{m}{2\pi}\int\ddd p\tilde{V}(\vec q-\vec p)\tilde{\psi_0}(\vec p)=-\frac{m}{2\pi}\tilde{V}(\vec q-k\uvec z).
\end{align}
或者直接写成内积的形式, 因为$\braket{p|q}=2\om p\dpi3\delta^3(\vec p-\vec q)$, 在非相对论条件下$\om p\approx m$, 于是
\begin{align}
    f(\vec q)=-\frac1{4\pi}\braket{q|V|p}\label{born-amplitude-V}.
\end{align}

然后我们考虑计算散射截面, 我们首先需要得到流到无穷远处的粒子流, 于是计算
\begin{align}
    \vec j=\frac1m\rm{Im}\left(\psi^*\nabla\psi\right), 
\end{align}
我们只考虑$j_r$, 并保留到$\frac 1{r^2}$项, 得到
\begin{align}
    j_r=\frac km\rm{Re}\left(\cos\theta+\frac{|f|^2}{r^2}+f\frac{\exp{ikr(1-\cos\theta)}}{r}+f^*\cos\theta\frac{\exp{-ikr(1-\cos\theta)}}{r}\right).
\end{align}
因为我们考虑的是$r\to\infty$的极限, 后两项干涉项$ikr\cos\theta$的相位将随角度迅速变化, 剧烈振荡, 在有限大小的探测器中会表现为强烈的干涉相消(除了$\theta=0$的情况, 这就是光学定律的由来), 只剩下
\begin{align}
    j_r=\frac km\left(\cos\theta+\frac{|f|^2}{r^2}\right).
\end{align}
显然第一项是入射粒子流的贡献, 第二项才是出射粒子流, 也就是
\begin{align}
    \vec j_{\rm{in}}=\frac km\uvec z, j_{r\rm{sc}}=\frac km\frac{|f|^2}{r^2}.
\end{align}
利用散射截面的定义
\begin{align}
    j_{\rm{in}}\d\sigma=j_{r\rm{sc}} r^2\d\Omega,
\end{align}
我们得到
\begin{align}
    \frac{\d\sigma}{\d\Omega}=|f(\theta, \varphi)|^2.
\end{align}
可见, 散射截面正比于散射振幅. 在一阶Born近似的情况下, 我们发现: 测量微分散射截面其实就是在测量势场的傅里叶变换谱.

然后我们可以讨论刚刚被我们略去的干涉项的物理贡献, 这就是光学定理. 光学定理的出发点其实很简单: 总共只有一个粒子在散射, 进入的粒子与出去的粒子流应当守恒. 既然粒子从直线运动$\exp{ikx}$变成了向各个方向散射, 那么为了保持总的粒子数还是不变的, 那么原本方向上的粒子流应当变暗. 更数学地说, 我们有粒子流守恒条件
\begin{align}
    \int_S\d\vec A\cdot\vec j=0.
\end{align}
这里$S$取$R\to\infty$的大球面. $\vec j_{\rm{in}}$的通量显然为$0$, 而利用总散射截面的定义可以得到散射项$\vec j_{\rm{sc}}$的通量
\begin{align}
    \int_S\d\vec A\cdot\vec j_{\rm{sc}}=\frac km\sigma_{\rm{total}}\neq0.
\end{align}
这告诉我们在计算微分散射截面时略去的两个干涉项一定对积分是有贡献的. 直接写出球面积分, 并先计算$\theta$向的积分, 利用$\d(1-\cos\theta)=\sin\theta\d\theta$, 设$u=1-\cos\theta$, 分部积分有
\begin{align}
    \int_S\d\vec A\cdot\vec j_{\rm{inf}}&=\frac im\int\d\varphi\rm{Re}\int \left(-if(u,\varphi)\d\exp{ikRu}+if^*(1-u)\d\exp{-ikRu}\right)\notag\\
    &=\frac{i}m\int\d\varphi\rm{Re}\Big(-f(u,\varphi)\exp{ikRu}\Big|_0^2+\int_0^2\exp{ikRu}\de fu\notag\\
    &\qquad+f^*(u,\varphi)\exp{-ikRu}-\int_0^2\exp{-ikRu}\de{}u\left((1-u)f\right)\Big).
\end{align}
因为当$R\to\infty$时$\exp{ikRu}$随$u$变化剧烈振荡, 因此第二项的积分结果应当干涉相消为$0$. 而第一项的当$u=2$时, 也随着$R$增大剧烈振荡, 对于一个有限大尺度的观测, 结果也应当为$0$, 于是就只剩下$u=0$也就是$\theta=0$处的贡献
\begin{align}
    \int_S\d\vec A\cdot\vec j_{\rm{inf}}&=-\frac{4\pi}m\rm{Im}f(\theta=0),
\end{align}
于是最终我们得到光学定理.
\begin{theorem}[光学定理]
    \begin{align}
        \sigma_{\rm{total}}=\frac{4\pi}k f(\theta=0).
    \end{align}
\end{theorem}
由此可见, 干涉项只会在$\theta=0$处有贡献, 用于干涉抵消原来的入射粒子流.

\newpage
\section{狭义相对论简述}
\subsection{Minkovski时空与Lorentz变换\label{sr-lorentz}}
在惯性参考系下度规张量为
\begin{align}
    g_{\mu\nu}=\rm{diag}(1, -1, -1, -1),
\end{align}
的时空称为Minkkovski时空. 狭义相对论研究这个时空流形中的动力学.
\begin{definition}[Lorentz变换]
    可以保持度规张量形式的坐标变换称为Lorentz变换.
\end{definition}
...
\begin{definition}[Lorentz标量]
    在Lorentz变换下不变的量称为Lorentz标量.
\end{definition}
\begin{theorem}
    物体静质量$m$是Lorentz标量.
\end{theorem}
\begin{definition}[Lorentz矢量]
    具有协变性的矢量称为Lorentz矢量.
\end{definition}
\begin{definition}[Lorentz张量]
    具有协变性的张量称为Lorentz张量.
\end{definition}
...
\begin{proof}
    ...
    计算可得:
    \begin{align}
        s+t+u&=E_1^2+E_2^2+E_3^2+E_4^2-2\vec p^2-2\vec k^2\notag\\
        &\quad+2E_1^2+2E_1E_2-2E_1E_3-2E_1E_4\\
